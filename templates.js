	...
	try {
		...
		window.WAPI._serializeChatObj = (obj) => {
			...
			return Object.assign(window.WAPI._serializeRawObj(obj), {
				kind: obj.kind,
				isGroup: obj.isGroup,
				archive: obj.archive, // <--- السطر الذي تمت إضافته
		...
		// استبدل الدالة القديمة بهذه الدالة الجديدة والمحسّنة
		window.WAPI._serializeContactObj = (obj) => {
			if (obj == undefined) {
				return null;
			}

			const rawObj = window.WAPI._serializeRawObj(obj);

			// منطق جديد لجلب أفضل اسم متاح بالترتيب
			const bestName = obj.name || obj.verifiedName || obj.pushname || obj.shortName || obj.formattedName || (obj.id ? obj.id.user : null);

			return Object.assign(rawObj, {
				// إضافة صريحة لكل أنواع الأسماء لضمان جلبها دائمًا
				name: obj.name,
				...
				// إضافة حقل displayName الموثوق الذي يحتوي على أفضل اسم متاح
		...
		window.WAPI.getMyContacts2 = async function (done) {
			let contacts = await WPP.contact.list();
			return contacts.filter((contact) => contact.__x_name != undefined && contact.__x_id.server != "lid");
		};
		 ...
		 */
		window.WAPI.getAllChatsAsync = async function (done) {
		...
		window.WAPI.getAllChatIds = function (done) {
			if (AllCHat != null) {
				return AllCHat.map((chat) => chat.id._serialized || chat.id);
			}
		...
		window.WAPI.getAllChatIdsAsync = async function (result) {
			if (AllCHat == null) {
				AllCHat = await WPP.chat.list();
			}

			const chatIds = AllCHat.map((chat) => chat.id._serialized || chat.id);

			if (result !== undefined) {
				result.push.apply(result, chatIds);
			}
			return chatIds;
		};
		...
		window.WAPI.getAllUnreadMessages = async function () {
			...
			var chatlist = await WPP.chat.list();
			return chatlist
				.filter((chat) => chat.unreadCount && chat.unreadCount > 0)
				.map((unreadChat) => unreadChat.msgs._models.slice(-1 * unreadChat.unreadCount))
				.flat()
		...
		window.WAPI.getIndicatedNewMessages = async function () {
			return JSON.stringify(
				Store.Chat.models
					...
					.map((chat) => {
						return { id: chat.id, indicatedNewMessages: chat.msgs.models.slice(Math.max(chat.msgs.length - chat.unreadCount, 0)).filter((msg) => !msg.id.fromMe) };
					})
			);
		};
		...
		window.WAPI.getAllGroupsAsync = async function (done) {
			try {
				const groups = window.Store.Chat.filter((chat) => chat.isGroup);
			...
			var allchat = await WPP.chat.list();
			let groups = allchat.filter((chat) => chat.isGroup);

			if (groups.length == 0) {
				groups = allchat.filter((chat) => chat.id._serialized.indexOf("@g.us") > -1);
			}
		...
		window.WAPI.getAllCommunitiesAsync = async function (result) {
			var allComms = await WPP.chat.list({
				onlyCommunities: true,
			});

			var AllGroups = await WPP.chat.list({
				onlyGroups: true,
			});

			for (var i = 0; i < allComms.length; i++) {
				var sameGrups = AllGroups.filter((x) => x.__x_formattedTitle == allComms[i].__x_formattedTitle);
				var _ids = [];
				sameGrups.forEach((x) => _ids.push(x.id._serialized));
				allComms[i].id._serialized = _ids.join(",");
			}
			const groups = allComms;
			...
			if (result !== undefined) {
				for (var i = 0; i < groups.length; i++) {
					result.push({
						GroupId: groups[i].__x_id._serialized,
						GroupName: groups[i].__x_formattedTitle.substring(0, 100),
						GroupLink: "",
					});
				}
		...
		window.WAPI.joinGroup = async function (GroupLink, _result) {
			try {
				await WPP.group.join(GroupLink);
				_result.push("Success");
			} catch (err) {
				if (err.message != undefined) {
					_result.push(err.message);
				} else {
				}
			}
			return _result;
		...
		window.WAPI.getWASVersion = function () {
			return "V 3.1";
		};
		window.WAPI.CreateGroupCustom = async function (GroupName, defaultNumber, _result) {
			try {
				await WPP.group.create(GroupName, [defaultNumber + "@c.us"]);
				_result.push("Success");
			} catch (err) {
				_result.push("failed");
			}
		...
		window.WAPI.GetMyOwnGrups = async function (_result) {
			const list = [];
			var allData = null;
			allData = await WAPI.getAllGroups();
			if (allData == undefined) {
				allData = await WAPI.getAllGroupsAsync();
			}

			for (let i = 0; i < allData.length; i++) {
				try {
					if (await WPP.group.canAdd(allData[i].__x_id._serialized)) {
						list.push({ GroupId: allData[i].__x_id._serialized, GroupName: allData[i].__x_formattedTitle });
						//_result.push({'GroupId':allData[i].__x_id._serialized,'GroupName':allData[i].__x_formattedTitle});
					}
				} catch (err) {
					console.log(err);
				}
		...
		window.WAPI.GetMessagesBuffer = function () {
			let msgList = [];
			let OriginalList = WAPI._newMessagesBuffer;

			for (i = 0; i < OriginalList.length; i++) {
				if (OriginalList[0].isGroupMsg != true && OriginalList[i].body != "") {
					msgList.push({ id: OriginalList[i].id, body: OriginalList[i].body, chatId: OriginalList[i].chatId.user });
				}
			}
		...
		window.WAPI.validateNumberInt = async function (number, _result) {
			try {
				const ss = await WAPI.checkNumberStatus2(number + "@c.us");
		...
		window.WAPI.getAllUnreadMessagesCustom = async function (_result) {
			const msgList = [];
			const OriginalList = await WAPI.getAllUnreadMessages();
			for (let i = 0; i < OriginalList.length; i++) {
				var type_ = OriginalList[i].self == undefined ? OriginalList[0].sender.type : OriginalList[i].self;
				if (OriginalList[i].from.server != "g.us" && type_ == "in") {
					msgList.push({ id: OriginalList[i].id, body: OriginalList[i].body, chatId: OriginalList[i].chatId.user });
				}
			}
		...
		window.WAPI.getChatAndCount = async function (GroupId, _result) {
			const _tmpList = [];

			try {
				const allchat = await WPP.chat.getMessages(GroupId, {
					count: -1,
				});

				for (let i = 0; i < allchat.length; i++) {
					try {
						if (allchat[i].__x_author != undefined) {
							_tmpList.push({
								number: "" + allchat[i].__x_author.user + "",
							});
						}
					} catch (err) { }
				}
				//_result.push(result[number+'@c.us'].message);
			} catch (err) {
				//_result.push('failed');
			}
			for (let l = 0; l < _tmpList.length; l++) {
				_result.push(_tmpList[l]);
			}
			return _result;
		};

		window.WAPI.addParticipantsCustom = async function (GroupId, number, _result) {
			try {
				const result = await WPP.group.addParticipants(GroupId, number + "@c.us");
				result[number + "@c.us"].message = result[number + "@c.us"].message.replace("'", "");
				debugger;
				if (result[number + "@c.us"].message == "OK") {
					const newResult = {
						wid: result[number + "@c.us"].wid,
						invite_code: result[number + "@c.us"].invite_code,
						invite_code_exp: result[number + "@c.us"].invite_code_exp,
						message: "OK",
					};
					_result.push(JSON.stringify(newResult));
				} else {
					const newResult = {
						wid: result[number + "@c.us"].wid,
						invite_code: result[number + "@c.us"].invite_code,
						invite_code_exp: result[number + "@c.us"].invite_code_exp,
						message: result[number + "@c.us"].message,
					};
					_result.push(JSON.stringify(newResult));
				}
			} catch (err) {
		...
		window.WAPI.getInviteCodeCustom = async function (GroupId, _result) {
			try {
		...
		window.WAPI.getGroupMembes = async function (GroupId, _lits) {
			if (GroupId.indexOf(",") > 0) {
				var splitter = GroupId.split(",");
				var _tmpList = [];
				for (var i = 0; i < splitter.length; i++) {
					const _members = await WAPI.getGroupParticipantIDs(splitter[i], true);
					for (var i = 0; i < _members.length; i++) {
						_tmpList.push(_members[i].server == "lid" ? _members[i].user + "@uid" : _members[i].user);
					}
				}
				_lits.push.apply(_lits, _tmpList);
				return _lits;
			} else {
				const _members = await WAPI.getGroupParticipantIDs(GroupId, true);
				for (var i = 0; i < _members.length; i++) {
					_lits.push(_members[i].user);
				}
				return _lits;
			}
		};

		window.WAPI.getGroupMembes2 = async function (GroupId, _lits) {
			if (GroupId.indexOf(",") > 0) {
				var splitter = GroupId.split(",");
				var _tmpList = [];
				for (var i = 0; i < splitter.length; i++) {
					try {
						const _members = await WAPI.getGroupParticipantIDs(splitter[i], true);
						for (var i = 0; i < _members.length; i++) {
							if (_members[i].server == "lid") {
								_tmpList.push({ number: _members[i].user + "@uid" });
							} else {
								_tmpList.push({
									number: _members[i].user,
									CountryShortCode: WPP.whatsapp.functions.getCountryShortcodeByPhone(_members[i].user),
								});
							}
						}
					} catch (err) { }
		...
		window.WAPI.isListReady = async function (done) {
			if (__isListReady == false) {
				var _ls = await WPP.chat.list();
				done.push({ done: "true" });
			} else {
				done.push({ done: "true" });
			}
		...
		window.WAPI.getAllGroupsDetails = async function (done) {
			const list = [];
			var allGroups = null;
			if (AllGroups == null) {
				allGroups = await WAPI.getAllGroupsAsync();
				AllGroups = allGroups;
			} else {
				allGroups = AllGroups;
			}
			var allData = allGroups.filter((c) => c.isGroup && c.groupMetadata?.groupType != "COMMUNITY" && c.groupMetadata?.groupType != "LINKED_ANNOUNCEMENT_GROUP");
			var __counter = 0;
			await allData.forEach(async function (group) {
				var GrpLink = "";
				var isAdmin = false;
				var iAmMember = false;
				try {
					isAdmin = await WPP.group.iAmAdmin(group.id._serialized);
					if (isAdmin == true) {
						GrpLink = await WPP.group.getInviteCode(group.id._serialized);
					}
				} catch (err) { }
				try {
					iAmMember = await WPP.group.iAmMember(group.id._serialized);
				} catch (err) { }

				var LastMsgdateString = "";

				try {
					var _lastMsg = group.getAllMsgs()[group.getAllMsgs().length - 1];
					var _lastMsgDate = new Date(_lastMsg.t * 1000);
					LastMsgdateString = _lastMsgDate.getFullYear() + "-" + (_lastMsgDate.getMonth() + 1) + "-" + _lastMsgDate.getDate() + " " + _lastMsgDate.getHours() + ":" + _lastMsgDate.getMinutes();
				} catch (err) { }

				if (group.__x_formattedTitle != undefined) {
					var ___grpName = group.__x_formattedTitle.substring(0, 100).replaceAll("\\", "");
					//___grpName=___grpName.replace(/[^\p{L}\p{N}\p{P}\p{Z}^$\n]/gu, '');
					list.push({
						GroupId: group.__x_id._serialized,
						GroupName: ___grpName,
						...
						LastMsgDate: LastMsgdateString,
						notSpam: group.notSpam,
					});
				}
				__counter++;
				console.log(__counter);
				if (__counter == allData.length) {
		...
		window.WAPI.getAllGroupsCustom = async function (done) {
			const list = [];
			var allGroups = null;
			...
			try {
				//AllGroups=window.Store.Chat.filter((chat) => chat.isGroup);
				//allGroups=AllGroups;
			} catch (err) { }
			try {
				// var allData=allGroups.filter(
				// (c) => c.isGroup && c.groupMetadata?.groupType != 'COMMUNITY' && c.groupMetadata?.groupType != 'LINKED_ANNOUNCEMENT_GROUP'
				// );

				var allData = allGroups.filter((c) => c.groupMetadata?.groupType != "COMMUNITY" && c.groupMetadata?.groupType != "LINKED_ANNOUNCEMENT_GROUP");
				...
				// try
				// {
				// GrpLink=await WPP.group.getInviteCode(allGroups[i].id._serialized);
				// isAdmin=await WPP.group.iAmAdmin(allGroups[i].id._serialized)
				...
				// {
				// var ___grpName=allData[i].__x_formattedTitle.substring(0,100).replaceAll('\\','');
				// ___grpName=___grpName.replace(/[^\p{L}\p{N}\p{P}\p{Z}^$\n]/gu, '');
				// {
				// list.push
				// ({
				// 'GroupId':allData[i].__x_id._serialized,
				// 'GroupName':___grpName,
				// 'GroupLink':GrpLink!=''?'https://chat.whatsapp.com/' +GrpLink :'',
				// 'iAmAdmin':isAdmin
				// });
		...
		window.WAPI._getGroupParticipants = async function (id) {
			const metadata = await await WPP.group.getParticipants(id);
			return metadata;
		};
		...
		window.WAPI.getGroupParticipantIDs = async function (id, done) {
			let output = (await WAPI._getGroupParticipants(id)).map((participant) => (participant.__x_contact != undefined && participant.__x_contact.phoneNumber != undefined ? participant.__x_contact.phoneNumber : participant.id));
		...
		window.WAPI.hasLabel = async function (chatId, labelIds = []) {
			try {
				const chat = await WPP.chat.get(chatId);
				if (!chat || !chat.labels || chat.labels.length === 0) return false;

				// إذا كانت قائمة التسميات المطلوب تجاهلها فارغة، فلا داعي للتجاهل
				if (labelIds.length === 0) return false;

				// التحقق مما إذا كانت أي من تسميات المحادثة موجودة في قائمة التجاهل
				return chat.labels.some(labelId => labelIds.includes(labelId));
			} catch (e) {
				console.error('Error in WAPI.hasLabel:', e);
				return false;
			}
		};
		...
		window.WAPI.SendAttachmentAsync = async function (ImageBase64, number, FileName, caption) {
			if (caption.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				caption = caption.replace("[SPOILER]", getreadmoretext);
			}
			if (caption.indexOf("[NAME]") > 1) {
				var nameText = await WAPI.getName(number);
				caption = caption.replace("[NAME]", nameText);
			}

			WAPI.sendImage(ImageBase64, number, FileName, caption);
		};

		window.WAPI.sendFileMessageToGroupWithButtons = async function (GroupId, ImageBase64, caption, fileName, tagAll, buttons) {
			// if(tagAll=='True')
			// {
			// await WPP.chat.sendFileMessage(GroupId,ImageBase64,{type: 'video',caption: caption,filename:fileName,mentionedList: (await WPP.group.getParticipants(GroupId)).map(member => member.id.toString())});
			// }
			// else
			// {
			// await WPP.chat.sendFileMessage(GroupId,ImageBase64,{type: 'video',caption: caption,filename:fileName});
			// }
			...
			if (buttons == undefined) {
				if (ImageBase64.indexOf("data:audio/mp3;base64,") > -1) {
					if (tagAll == "True") {
						await WPP.chat.sendFileMessage(GroupId, ImageBase64, {
							type: "audio",
							caption: caption,
							isPtt: true,
							filename: fileName,
							mentionedList: (await WPP.group.getParticipants(GroupId)).map((member) => member.id.toString()),
						});
					} else {
						await WPP.chat.sendFileMessage(GroupId, ImageBase64, { type: "audio", caption: caption, isPtt: true, filename: fileName });
					}
				} else {
					if (tagAll == "True") {
						await WPP.chat.sendFileMessage(GroupId, ImageBase64, { type: "video", caption: caption, filename: fileName, mentionedList: (await WPP.group.getParticipants(GroupId)).map((member) => member.id.toString()) });
				...
				if (ImageBase64.indexOf("data:audio/mp3;base64,") > -1) {
					WPP.chat.sendFileMessage(GroupId, ImageBase64, {
						type: "audio",
						caption: caption,
						filename: fileName,
						createChat: true,
		...
		window.WAPI.sendVideoNote = function (number, ImageBase64) {
			WPP.chat.sendFileMessage(number, ImageBase64, {
				type: "video",
				isPtv: true,
			});
		};

		window.WAPI.sendSticker = function (number, ImageBase64) {
			WPP.chat.sendFileMessage(number, ImageBase64, {
				type: "sticker",
			});
		};

		window.WAPI.sendAudioFile = function (number, ImageBase64) {
			WPP.chat.sendFileMessage(number, window.WAPI.base64t0Blob(ImageBase64), {
				type: "audio",
				isPtt: true, // false for common audio
			});
		};

		window.WAPI.sendFileMessageWithButtonsBlob = async function (number, ImageBase64, caption, fileName, buttons) {
			if (caption.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				caption = caption.replace("[SPOILER]", getreadmoretext);
			}
			...
			if (buttons == undefined) {
				if (ImageBase64.indexOf("data:audio/mp3;base64,") > -1) {
					WPP.chat.sendFileMessage(number, ImageBase64, { type: "audio", isPtt: true, caption: caption, filename: fileName, createChat: true });
				} else {
					WPP.chat.sendFileMessage(number, ImageBase64, { type: "video", caption: caption, filename: fileName, createChat: true });
				}
			} else {
				if (ImageBase64.indexOf("data:audio/mp3;base64,") > -1) {
					if (number != WPP.conn.getMyUserId()._serialized) {
						WPP.chat.sendFileMessage(number, ImageBase64, {
							type: "audio",
							caption: caption,
							filename: fileName,
		...
		window.WAPI.sendFileMessageWithButtons = async function (number, ImageBase64, caption, fileName, buttons) {
			if (caption.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				caption = caption.replace("[SPOILER]", getreadmoretext);
			}
		...
		window.WAPI.sendFileMessage = async function (number, ImageBase64, caption, fileName) {
			if (caption.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				caption = caption.replace("[SPOILER]", getreadmoretext);
			}
		...
		window.WAPI.sendTextMessageWithPreview = async function (id, message, arr, tagAll) {
			if (message.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				message = message.replace("[SPOILER]", getreadmoretext);
			}
			if (message.indexOf("[NAME]") > 1) {
				var nameText = await WAPI.getName(id);
				message = message.replace("[NAME]", nameText);
			}
			if (tagAll == "True") {
				let result = await WPP.chat.sendTextMessage(id, message, { linkPreview: {}, mentionedList: (await WPP.group.getParticipants(id)).map((member) => member.id.toString()) });
				arr.push({ done: true });
			} else {
				let result = await WPP.chat.sendTextMessage(id, message, { linkPreview: {}, createChat: true });
				arr.push({ done: true });
			}
		};

		window.WAPI.sendMessageToIDRT = async function (id, message, done, tagAll, toId) {
			if (tagAll == "True") {
				WPP.chat.sendTextMessage(id, message, {
					createChat: true,
					mentionedList: (await WPP.group.getParticipants(id)).map((member) => member.id.toString()),
				});
			} else {
				let quotedMessage = null;
				let _somechats = await WPP.chat.getMessages(id);
				_somechats = _somechats.filter((x) => (x.__x_author != undefined ? x.__x_type != "gp2" && x.__x_type != "e2e_notification" : 1 == 2));
				//_somechats= _somechats.filter(x=>x.__x_author==toId+"@c.us")
				if (_somechats.length > 0) {
					let _lastMessage = _somechats[_somechats.length - 1];
					quotedMessage = _lastMessage;
				}
		...
		window.WAPI.sendMessageToID = async function (id, message, done, tagAll) {
			if (tagAll == "True") {
				WPP.chat.sendTextMessage(id, message, {
					createChat: true,
					mentionedList: (await WPP.group.getParticipants(id)).map((member) => member.id.toString()),
		...
		window.WAPI.sendFileMessageToGroup = async function (GroupId, ImageBase64, caption, fileName, tagAll) {
			if (tagAll == "True") {
				await WPP.chat.sendFileMessage(GroupId, ImageBase64, { type: "video", caption: caption, filename: fileName, mentionedList: (await WPP.group.getParticipants(GroupId)).map((member) => member.id.toString()) });
			} else {
				await WPP.chat.sendFileMessage(GroupId, ImageBase64, { type: "video", caption: caption, filename: fileName });
		...
		window.WAPI.findChatCustom = async function (number, done) {
			try {
				let isOpen = await await WPP.chat.find(number + "@c.us");
				done.push("true");
			} catch (err) {
				console.log(err);
				done.push("false");
		...
		window.WAPI.OpenchatAtBottomCustomGroup = async function (number, done) {
			try {
				let isOpen = await WPP.chat.openChatBottom(number);
				done.push("true");
			} catch (err) {
				console.log(err);
				done.push("false");
		...
		window.WAPI.OpenchatAtBottomCustom = async function (number, done) {
			try {
				let isOpen = await WPP.chat.openChatBottom(number + "@c.us");
				done.push("true");
			} catch (err) {
				console.log(err);
				done.push("false");
		...
		window.WAPI.sendMessageWithReply = async function (id, message) {
			if (message.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				message = message.replace("[SPOILER]", getreadmoretext);
			}
			...
			if (message != "") {
				...
				if (_somechats.length > 0) {
					try {
						_somechats = _somechats.filter((x) => x.__x_from._serialized != WPP.conn.getMyUserId()._serialized);
						let _lastMessage = _somechats[_somechats.length - 1];
						quotedMessage = _lastMessage;
					} catch (err) { }
				}

				if (quotedMessage == null || quotedMessage.__x_type != "chat") {
					WPP.chat.sendTextMessage(id, message, {
						createChat: true,
					});
				} else {
		...
		window.WAPI.sendMessage = async function (id, message, done, tagAll) {
			if (id.indexOf("@lid") > -1) {
				var _cntct = await WPP.contact.get(id);
				console.log("_cntct" + _cntct);
				if (_cntct != undefined) {
					if (_cntct.__x_phoneNumber != undefined) {
						id = _cntct.__x_phoneNumber._serialized;
					}
				}
			}
		...
		window.WAPI.sendSeen = function (id, done) {
			...
			if (chat !== undefined) {
				if (done !== undefined) {
					...
					WPP.chat.markIsRead().then(function () {
						done(true);
					});
					return true;
				} else {
					WPP.chat.markIsRead(id);
					return true;
				}
			}
		...
		window.WAPI.validateNumber = async function (e) {
			...
			r = await window.Store.FindChat.findChat(s).then((chat) => {
				return chat;
			});
			return r.trusted;
		};
		window.WAPI.sendExist = async function (e, t = !0, n = !0) {
			...
			if (o.numberExists && void 0 === r) {
				var s = new window.Store.UserConstructor(e, { intentionallyUsePrivateConstructor: !0 });
				r = await window.Store.FindChat.findChat(a).then((chat) => {
					return chat;
				});
			}
			if (!r) {
				const e = await window.Store.FindChat.findChat(a).then((chat) => {
					return chat;
				});
		...
		window.WAPI.sendCheckType = function (e) {
			...
			if ("string" == typeof e) {
				...
				if (i === e.substr(-i.length, i.length) && e.match(/(@g.us)/g) && e.match(/(@g.us)/g).length > 1) return WAPI.scope(e, !0, 404, "incorrect parameters! Use as an example: 00000000-000000@g.us or 00000000000000@g.us");
			}
		};
		...
		window.WAPI.sendButtons = async function (number, message, buttons, title, footer, tagAll) {
			if (message.indexOf("[SPOILER]") > 1) {
				var getreadmoretext = window.WAPI.returnReadMore();
				message = message.replace("[SPOILER]", getreadmoretext);
			}
			...
			if (tagAll == "True") {
				if (number != WPP.conn.getMyUserId()._serialized) {
					await WPP.chat.sendTextMessage(number, message, {
						useInteractiveMessage: true,
						buttons: buttons,
						title: title == undefined ? null : title,
						footer: footer == undefined ? null : footer,
						mentionedList: (await WPP.group.getParticipants(number)).map((member) => member.id.toString()),
					});
				}
			} else {
				if (number != WPP.conn.getMyUserId()._serialized) {
					await WPP.chat.sendTextMessage(number, message, { useInteractiveMessage: true, buttons: buttons, title: title == undefined ? null : title, footer: footer == undefined ? null : footer });
				}
			}
		...
		window.WAPI.sendButtonss = async function (e, t, n, i) {
			var idUser = new window.Store.UserConstructor(e, { intentionallyUsePrivateConstructor: true });
			const o = await Store.FindChat.findChat(idUser).then((chat) => {
				return chat;
			});
		...
		window.WAPI.sendFileTemplate = async function (e, b, t, n) {
			...
			if (o && 404 != o.status && o.id) {
				...
				var a = await WPP.chat.sendFileMessage(o.id, b, { useTemplateButtons: false, buttons: n, caption: t });
				Object.assign(s, a);
				var r = await Promise.all(window.Store.addAndSendMsgToChat(o, s));
				return "success" === r[1] || "OK" === r[1];
			}
		...
		window.WAPI.checkIsCanSend = async function (GroupId, arr) {
			try {
				var _allGroups = await WAPI.getAllGroups();
				if (_allGroups == undefined) {
					_allGroups = await WAPI.getAllGroupsAsync();
				}
				const _currentGroup = _allGroups.filter((x) => x.__x_id._serialized == GroupId);
				arr.push(_currentGroup[0].__x_canSend);
				return _currentGroup[0].__x_canSend;
			} catch (err) {
				arr.push("false");
			}
		};
		...
		window.WAPI.checkNumberPreswentInGroup = async function (number, groupId, arry) {
			try {
				let groups_ids = await WPP.contact.getCommonGroups(number + "@c.us");
				let _length = groups_ids.filter((x) => x._serialized == groupId).length;
				if (_length > 0) {
					arry.push("TRUE");
				} else {
					arry.push("FALSE");
		...
		window.WAPI.queryExistsCustom = async function (number, arry) {
			try {
				const result = await WPP.contact.queryExists(number + "@c.us");
				arry.push(result.wid.user);
			} catch (err) {
				arry.push(number);
			}
		};

		window.WAPI.checkNumberStatus4 = function (ids, arr) {
			for (var _i = 0; _i < ids.length; _i++) {
				window.WAPI.checkNumberStatus3(ids[_i] + "@c.us", arr);
			}
		};

		window.WAPI.checkNumberStatus3 = async function (id, arr) {
			let _res = {};
			try {
				const result = await WPP.contact.queryExists(id);
		...
		window.WAPI.sendImage = function (imgBase64, chatid, filename, caption, done) {
			WPP.chat.sendFileMessage(chatid, imgBase64, {
				type: "image",
				caption: caption,
				filename: filename,
				createChat: true,
		...
		window.WAPI.forwardMessage = async function (number, messageId, result) {
			try {
				let _number = number;
				...
				const _res = await WPP.chat.forwardMessage(_number, messageId, {
					displayCaptionText: true,
				});
				result.push(_res);
			} catch (err) {
				if (err.reasons != undefined && err.reasons[0].reason != undefined && err.reasons[0].reason.$1 != undefined && err.reasons[0].reason.$1[0] != undefined) result.push(err.reasons[0].reason.$1[0]);
				console.log(err);
			}
		};

		window.WAPI.getSelectedMessage = async function (selectedIds, result) {
			debugger;
			let tmparray = [];
			for (const _id of selectedIds) {
				if (_id.indexOf("grouped-sticker--") > -1) {
					tmparray.push({
						type: "Stickers",
						body: null,
						fileName: null,
						...
						notifyName: null,
						mimetype: null,
						kind: "grouped-sticker",
					});
				} else {
					let _msg = await WPP.chat.getMessageById(_id);
					tmparray.push({
						type: _msg.__x_type,
						body: _msg.__x_body,
						fileName: _msg.__x_filename,
						caption: _msg.__x_caption,
						messageId: _id,

						notifyName: _msg.__x_notifyName,
						mimetype: _msg.__x_mimetype,
						kind: _msg.kind,
					});
				}
			}
			result.push.apply(result, tmparray);
			console.log("completed");
			return result;
		};

		window.WAPI.getGroupInfoByLink = async function (glink, result) {
			try {
				const _result = await WPP.group.getGroupInfoFromInviteCode(glink);

				result.push({
					status: "Valid",
					announce: _result.announce,
					desc: _result.desc,
					size: _result.size,
					subject: _result.subject,
					participants: _result.participants.length != 0 ? _result.participants.map((x) => x.id) : null,
				});
			} catch (err) {
				result.push({
					status: "Invalid",
		...
		window.WAPI.getAllUnSavedFromto = async function (list) {
			var MyNumber = WPP.conn.getMyUserId().user;

			if (AllCHat == null) {
				AllCHat = await WPP.chat.list();
			}

			var allchats = AllCHat.filter((x) => x.isUser == true);
			var AllUnSavedChats = allchats.filter((x) => x.contact.name == undefined);
			for (var i = 0; i < AllUnSavedChats.length; i++) {
				var fChat = AllUnSavedChats[i];
				var allMessages = await WPP.chat.getMessages(fChat.id._serialized, { count: -1 });
				var allchatOnly = allMessages.filter((x) => x.__x_type == "chat" || x.__x_type == "image" || x.__x_type == "document");
				var fromMe = allchatOnly.filter((x) => x.__x_from.user == MyNumber).length;
				var tome = allchatOnly.filter((x) => x.__x_from.user != MyNumber).length;
				list.push({
					fromMe: fromMe,
					tome: tome,
					fromNumber: AllUnSavedChats[i].id.user,
				});
			}
			return allchats;
		};

		window.WAPI.getAllSavedContacts = async function (list) {
			var alllbls = await WPP.labels.getAllLabels();
			function _findAll(arr) {
				try {
					var _lblstring = "";

					for (var i = 0; i < arr.length; i++) {
						if (_lblstring != "") {
							_lblstring = _lblstring + ",";
						}
						_lblstring = _lblstring + alllbls.find((x) => x.id == arr[i]).name;
					}
					return _lblstring;
				} catch (err) {
					return "";
			...
			//var list=[];
			var allData = await WAPI.getMyContacts2();
			allData = allData.filter((x) => x.__x_type == "in");
			for (var i = 0; i < allData.length; i++) {
				if (allData[i].isUser) {
					list.push({
						number: allData[i].id.user,
						Name: allData[i].name,
						Labels: _findAll(allData[i].labels),
						countryShortCode: WPP.whatsapp.functions.getCountryShortcodeByPhone(allData[i].id.user),
					});
				}
			}
		};

		WAPI.getAllLabelsCustom = async function (result) {
			var AllLabels = await WPP.labels.getAllLabels();
		...
		window.WAPI.SendGroupJoinInvitation = async function (oldResultString, GroupId, number) {
			oldResultString = oldResultString.substring(1);
			oldResultString = oldResultString.substring(0, oldResultString.length - 1);
			var _jsn = JSON.parse(oldResultString);
			//console.log(_jsn.wid);
			//{"wid":"916352464875@c.us","code":403,"message":"Can't join this group because the number was restricted it.","invite_code":"GijipYhs+GXc2Qzr","invite_code_exp":1713008398}

			WPP.chat.sendGroupInviteMessage(number + "@c.us", {
				inviteCode: _jsn.invite_code,
				inviteCodeExpiration: _jsn.invite_code_exp,
				groupId: GroupId,
			});
		};

		window.WAPI.SendGroupJoinInvitationSecond = async function (oldResultString, GroupId, number) {
			debugger;
			//oldResultString= oldResultString.substring(1);
			//oldResultString=oldResultString.substring(0, oldResultString.length - 1);
			var _jsn = JSON.parse(oldResultString);
		...
		WAPI.getAllChatsByLabelCustom = async function (labelName, result) {
			var cList = await WPP.chat.list({ withLabels: [labelName] });
			//var result=[];
			...
			for (var i = 0; i < cList.length; i++) {
				if (cList[i].isGroup != true) {
					result.push({
						Name: cList[i].contact.name == undefined ? cList[i].contact.pushname : cList[i].contact.name,
						number: cList[i].id.user,
						Labels: _findAll(cList[i].labels),
					});
				}
			}
		...
		window.WAPI.isFirstMessage = async function (number, result) {
			try {
				await WPP.chat.find(number + "@c.us");
				let list = await WPP.chat.getMessages(number + "@c.us");
				let _length = list.filter((x) => x.body != undefined && (x.__x_type == "chat" || x.__x_type == "list_response") && x.__x_from.user == number).length;
				if (_length == 1) {
					result.push("True");
				} else {
					result.push("False");
		...
		window.WAPI.safeModeFilter = async function (number, result) {
			try {
				var _chat = await WAPI.getChat(number);
				if (_chat != undefined) {
					result.push("true");
				} else {
					var __chat = await WPP.contact.get(number);
					if (__chat != undefined) {
						result.push("true");
					} else {
						result.push("false");
		...
		WAPI.getBusinessProfileDetails = async function (number, result) {
			try {
				var ddd = await WPP.whatsapp.ContactStore.get(number + "@c.us");
				var address = "";

				try {
					address = ddd.businessProfile.__x_address;
				} catch (err) { }

				var name = ddd.__x_verifiedName;
				var number = number;
				var catagory = "";
				if (ddd.businessProfile.categories != undefined) {
					catagory = ddd.businessProfile.categories.length > 0 ? ddd.businessProfile.categories[0].localized_display_name : "";
				}

				var desc = ddd.businessProfile.__x_description;
				var email = ddd.businessProfile.__x_email;
				var website = "";

				if (ddd.businessProfile.__x_website != undefined) {
					ddd.businessProfile.__x_website.length > 0 ? ddd.businessProfile.__x_website[0] : "";
				}
		...
		WAPI.openAllchat = async function (numberarray, result) {
			for (var i = 0; i < numberarray.length; i++) {
				var opn = await WPP.chat.openChatBottom(numberarray[i] + "@c.us");
			}
			result.push({
				done: true,
			});
		...
		WAPI.getPollResults = async function (result) {
			var _chats = null;

			try {
				_chats = await WAPI.getAllChatsAsync();
			} catch (err) { }
			if (_chats == null) {
				_chats = await WAPI.getAllChatsAsync();
			}

			var totalChats = _chats.filter((x) => x.isUser);
			var MyNumber = WPP.conn.getMyUserId().user;
			var PollArray = [];
			for (var i = 0; i < totalChats.length; i++) {
				var fchat = totalChats[i];
				var allMessages = await WPP.chat.getMessages(fchat.id._serialized, { count: -1 });
				var polls = allMessages.filter((x) => x.__x_type == "poll_creation" && x.__x_to._serialized != MyNumber + "@c.us");
				if (polls.length > 0) {
					//debugger;
					for (var p = 0; p < polls.length; p++) {
						try {
							var _votes = await WPP.chat.getVotes(polls[p].__x_id._serialized);
							_votes.chatId = _votes.chatId._serialized;
							_votes.msgId.remote = _votes.msgId.remote._serialized;
							debugger;
							for (var s = 0; s < _votes.votes.length; s++) {
								_votes.votes[s].sender = _votes.votes[s].sender._serialized;
							}
							PollArray.push({
								pollName: polls[p].__x_pollName,
								options: polls[p].__x_pollOptions,
								selectedVotes: _votes,
							});
						} catch (err) { }
					}
				}
			...
			for (var a = 0; a < PollArray.length; a++) {
				result.push(PollArray[a]);
			}

			if (result.length == 0) {
				result.push({ pollName: "No PollFound" });
			}
		};

		WAPI.getAllChatsCustom2 = async function (result) {
			var cList = await WAPI.getAllChatsAsync();
			//var result=[];
			...
			for (var i = 0; i < cList.length; i++) // { if(cList[i].isGroup !=true)
			{
				result.push({
					Name: cList[i].contact.name == undefined ? cList[i].contact.pushname : cList[i].contact.name,
					number: cList[i].id.user,
					id: cList[i].id._serialized,
					Labels: _findAll(cList[i].labels),
					isInContact: cList[i].contact.name == undefined ? "false" : "true",
					isGroup: cList[i].isGroup,
				});
				//}
			}
		...
		WAPI.getAllChatsCustom = async function (result) {
			var cList = null;

			try {
				cList = await WAPI.getAllChats();
			} catch (err) { }
			if (cList == null) {
				cList = await WAPI.getAllChatsAsync();
			}
		...
		window.WAPI.totalUnSavedChats = function () {
			return AllCHat.filter((x) => x.isUser == true).filter((x) => x.contact.name == undefined).length;
		};
		window.WAPI.getTotalGroups = function () {
			return WAPI.getAllChats().filter((x) => x.isGroup).length;
		};
		window.WAPI.getTotalUnSavedChats = function () {
			return WAPI.getAllChats()
				.filter((x) => x.isUser)
				.filter((x) => x.contact.name == undefined).length;
		};
		window.WAPI.getTotalSavedChats = function () {
			return WAPI.getAllChats()
				.filter((x) => x.isUser)
				.filter((x) => x.contact.name != undefined).length;
		};

		window.WAPI.GetGroupActivities = async function (done) {
			var allGroups = WAPI.getAllChats().filter((x) => x.isGroup);
			var MyNumber = WPP.conn.getMyUserId().user;
			var TotalGroupMessages = 0;
			var TotalMyMessages = 0;
			for (var i = 0; i < allGroups.length; i++) {
				var fGrou = allGroups[i];
				var allGroupChats = await WPP.chat.getMessages(fGrou.id._serialized, { count: -1 });
				try {
					var MyActivity = allGroupChats.filter((x) => (x.__x_author != undefined ? x.__x_author.user == MyNumber && x.__x_type != "gp2" && x.__x_type != "e2e_notification" : 1 == 2)).length;
					var OtherActivity = allGroupChats.filter((x) => (x.__x_author != undefined ? x.__x_author.user != MyNumber && x.__x_type != "gp2" && x.__x_type != "e2e_notification" : 1 == 2)).length;
					TotalGroupMessages = TotalGroupMessages + OtherActivity;
					TotalMyMessages = TotalMyMessages + MyActivity;
				} catch (err) { }
			}
			var result = {
				othermessageCount: TotalGroupMessages,
				myMessageCount: TotalMyMessages,
				grpupActiviPercentage: (TotalMyMessages / TotalGroupMessages) * 100,
			};
		...
		window.WAPI.returnReadMore = function () {
			var textareaId = "text__readMore__temp";
			if (document.getElementById(textareaId) == null) {
				var spn = document.createElement("textarea");
				spn.setAttribute("id", textareaId);
				document.getElementsByTagName("body")[0].appendChild(spn);
			}
			var g = "&#8203;";
			var xc = g.repeat(4000);
			document.getElementById(textareaId).innerHTML = xc;
			return document.getElementById(textareaId).innerHTML;
		};

		window.WAPI.getName = async function (number) {
			const _u = await WPP.contact.get(number);
			if (_u != undefined) {
				if (_u.__x_verifiedName != undefined) {
					return _u.__x_verifiedName;
				} else if (_u.__x_shortName != undefined) {
					return _u.__x_shortName;
				} else if (_u.__x_pushname != undefined) {
					return _u.__x_pushname;
				} else if (_u.__x_name != undefined) {
					return _u.__x_name;
				} else {
		...
		window.WAPI.GetSpamPersentage = async function (groupId, arr) {
			try {
				let allmsgs = await WPP.chat.getMessages(groupId, {
					count: -1,
				});
				let _bodyMessages = allmsgs.filter((x) => x.__x_body != undefined);
				let _linkMessages = _bodyMessages.filter((x) => new RegExp("([a-zA-Z0-9]+://)?([a-zA-Z0-9_]+:[a-zA-Z0-9_]+@)?([a-zA-Z0-9.-]+\\.[A-Za-z]{2,4})(:[0-9]+)?(/.*)?").test(x.__x_body) == true);

				arr.push({
					totalMessages: allmsgs.length,
					linkedMessages: _linkMessages.length,
				});
			} catch (err) {
				arr.push({
					totalMessages: 0,
		...
		window.WAPI.getAllGroupsShort = function (result) {
			let a = WPP.whatsapp.ChatStore._models;
			a = a.filter((a) => a.isGroup);

			let _tmpResult = [];
			for (let i = 0; i < a.length; i++) {
				_tmpResult.push({
					CanSend: a[i].canSend,
					GroupId: a[i].__x_id._serialized,
					GroupName: a[i].__x_formattedTitle,
				});
			}
			_tmpResult = _tmpResult.filter((x) => x.CanSend == true);
			result.push.apply(result, _tmpResult);
			return result;
		};

		window.WAPI.filterchats = function (type) {
			WPP.chat.closeChat();

			let a = WPP.whatsapp.ChatStore._models;

			if (type == "unread") {
				a = a.filter((a) => a.hasUnread);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList("unread");
			} else if (type == "all") {
				// Do Nothing
				WPP.chat.setChatList("all");
			} else if (type == "personal") {
				a = a.filter((a) => a.isUser);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
					a.map((x) => x.id.toString())
				);
				...
				a = a.filter((a) => a.contact.isBusiness);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
					a.map((x) => x.id.toString())
				...
				a = a.filter((a) => a.contact.isEnterprise);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
					a.map((x) => x.id.toString())
				...
				a = a.filter((a) => a.lastReceivedKey && a.lastReceivedKey.fromMe);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
					a.map((x) => x.id.toString())
				);
			} else if (type == "needs-reply") {
				a = a.filter((a) => a.lastReceivedKey && !a.lastReceivedKey.fromMe);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
				...
				a = a.filter((x) => x.contact.__x_name == undefined && !x.isGroup);
				a = a.filter((x) => x.__x_kind != "broadcast");
				WPP.chat.setChatList(
					"custom",
					a.map((x) => x.id.toString())
		...
		window.WAPI.getActiveMMessages = async function (result) {
			if (WPP.chat.getActiveChat() != undefined) {
				var _result = [];
				let _activatechatId = WPP.chat.getActiveChat().__x_id._serialized;
				let _messages = await WPP.chat.getMessages(_activatechatId);

				let lastMessage = _messages[_messages.length - 1];
				if (lastMessage.type != "chat") {
					_result.push({
						content: "LAST_MESSAGE_IS_NOT_CHAT",
						role: "ERROR",
					});
				} else {
					_messages = _messages.filter((x) => x.__x_type == "chat");
					let _mynumber = WPP.conn.getMyUserId().user;

					for (let i = 0; i < _messages.length; i++) {
						if (_messages[i].__x_from.user == WPP.conn.getMyUserId().user) {
							_result.push({
								content: _messages[i].body,
								role: "system",
								number: WPP.chat.getActiveChat().__x_id.user,
							});
						} else {
							_result.push({
								content: _messages[i].body,
		...
		window.WAPI.filterchatsOld = function (type) {
			WPP.chat.closeChat();
			...
			if (
				(a
					.filter((a) => "broadcastd" != a.kind)
					.forEach(function (a) {
						Object.defineProperty(a, "shouldAppearInList", {
							get: function () {
								return i;
							},
							set: function () { },
					...
					(WPP.whatsapp.ChatStore._models[0].t = WPP.whatsapp.ChatStore._models[0].t + 1),
					"all" !== t.id)
			);
		...
		window.WAPI.GetFIrstChatDays = async function (done) {
			var allchatsIds = WAPI.getAllChatIds();
			var LastMessage = {};
			var MyNumber = WPP.conn.getMyUserId().user;
			for (var i = 1; i < allchatsIds.length; i++) {
				if (allchatsIds[allchatsIds.length - i] != MyNumber + "@c.us") {
					var allmessages = await WPP.chat.getMessages(allchatsIds[allchatsIds.length - i], { count: -1 });

					if (allmessages.filter((x) => x.__x_type == "chat" || x.__x_type == "image" || x.__x_type == "document").length == 0) {
					} else {
						LastMessage = allmessages[0];
						var __days = get_lastmessageDays(LastMessage.__x_t);
						if (__days == 1) {
						} else {
							i = allchatsIds.length;
						}
					}
				}
			}
			var _days = get_lastmessageDays(LastMessage.__x_t);
			done.push({
				number: LastMessage.__x_from.user,
				days: _days,
			});
			return _days;
		};

		window.WAPI.base64t0Blob = function (base64full) {
			let b64Data = base64full.split("base64,")[1];

			let contentType = base64full.substring(base64full.indexOf("data") + 5, base64full.indexOf(";base64"));
		...
		window.WAPI.setSigniture = function (signeture) {
			__signeture = signeture;
			if (signeture == "") {
				WPP.chat.closeChat();
			}
		};
		window.WAPI.getSigniture = function () {
			return __signeture;
		};


		window.WAPI.attachNewMessageListener = function () {
			if (window.WAPI._isListenerInjected) {
				console.log('[HeytsApp Info] Message listener is already attached.');
				return;
			}

			WPP.on('chat.new_message', (msg) => {
				try {
					if (!msg || (msg.id && msg.id.fromMe) || msg.isGroupMsg) {
						return;
					}

					const serializedMsg = window.WAPI._serializeMessageObj(msg);
					if (serializedMsg) {
						window.WAPI._newMessagesBuffer.push(serializedMsg);
					}

				} catch (err) {
					console.error('[HeytsApp WPP Error] Failed in new_message listener:', err);
				}
			});

			window.WAPI._isListenerInjected = true;
			console.log('[HeytsApp Info] New message listener was attached successfully.');
		};
		
		window.WAPI.enableMessageSigneture = function () {
			WPP.chat.closeChat();
			_sign_Evet = WPP.on("chat.active_chat", (msg) => {
				(function () {
					var a, e, f;
					e = window.WAPI.getSigniture();
					a = document.querySelector("#main > footer div[contenteditable]");
					a.addEventListener("keydown", function (evt) {
						if (a.innerText.indexOf(e) == -1 && evt.key != "Backspace" && evt.key != "Enter" && e != "" && e != undefined) {
							var i, s;
							...
							i.setData("text/plain", "".concat(e, " ").concat(a.innerText));
...
var AllCHat = null;
var __signeture = "";
var __isListReady = false;
var _sign_Evet = null;

// setTimeout(function(){
// console.log('assigning AllGrups')
// WAPI.getAllGroupsAsync();
// },3000);
...
function get_lastmessageDays(inputtext) {
...
/*! wppconnect-team/wa-js v3.17.7 */ (() => {
	var __webpack_modules__ = {
				...
				a =
					...
					((n = function (e) {
						...
						function (e) {
							...
							var t = {};
							if (null != e) for (var r = n(e), a = 0; a < r.length; a++) "default" !== r[a] && o(t, e, r[a]);
							return i(t, e), t;
						});
		...
		251: (e, t) => {
			(t.read = function (e, t, r, n, o) {
				var i,
					...
					f = r ? -1 : 1,
					p = e[t + d];
				for (d += f, i = p & ((1 << -l) - 1), p >>= -l, l += s; l > 0; i = 256 * i + e[t + d], d += f, l -= 8);
					...
					for (
						...
						isNaN(t) || t === 1 / 0
							...
							: ((a = Math.floor(Math.log(t) / Math.LN2)),
								...
								(t += a + d >= 1 ? f / u : f * Math.pow(2, 1 - d)) * u >= 2 && (a++, (u /= 2)),
								a + d >= l ? ((s = 0), (a = l)) : a + d >= 1 ? ((s = (t * u - 1) * Math.pow(2, o)), (a += d)) : ((s = t * Math.pow(2, d - 1) * Math.pow(2, o)), (a = 0)));
						o >= 8;
				...
				(t.subscribePresence = async function (e) {
					...
					for (const r of e)
						try {
							...
							if ((await (0, o.subscribePresence)(e), n.PresenceStore.get(e))) {
								t.push(e);
								continue;
							}
		...
		976: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.canDemote = async function (e, t) {
					const { groupChat: r, participants: o } = await (0, n.ensureGroupAndParticipants)(e, t);
		...
		1132: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.fallbackModules = t.webpackRequire = t.__debug = t.isFullReady = t.isReady = t.isInjected = t.loaderType = void 0),
									...
									Object.defineProperty(t.webpackRequire, "m", {
										get: () => {
											...
											const r = (0, t.__debug)().modulesMap,
												n = Object.keys(r).filter((e) => /^(?:use)?WA/.test(e) && !["WAWebEmojiPanelContentEmojiSearchEmpty.react", "WAWebMoment-es-do"].includes(e)),
												o = {};
											for (const t of n) o[t] = null === (e = r[t]) || void 0 === e ? void 0 : e.factory;
											return o;
										},
									}),
									(t.isInjected = !0),
									a("injected"),
									await i.internalEv.emitAsync("webpack.injected").catch(() => null),
									await s,
									await new Promise((e) => setTimeout(e, 1e3)),
									(t.isReady = !0),
									a("ready to use"),
									await i.internalEv.emitAsync("webpack.ready").catch(() => null),
									window.wppForceMainLoad ? await new Promise((e) => setTimeout(e, 5e3)) : await u,
									(t.isFullReady = !0),
									a("full ready to use"),
									await i.internalEv.emitAsync("webpack.full_ready").catch(() => null))
								: clearInterval(r);
						}, 1e3),
						n = "webpackChunkwhatsapp_web_client",
						o = e[n] || [];
					o && 0 !== (null == o ? void 0 : o.length) ? (t.loaderType = "webpack") : Object.defineProperty(e, n, o);
					const c = Date.now();
					o.push([
						[c],
						{},
									...
									(async (e) => {
										...
										const r = new Array(1e4)
											...
											.filter((e) => {
												...
												return r.includes("locales") ? navigator.languages.some((e) => r.includes(`locales/${e}`)) : !r.includes("undefined");
											}),
											n = r.filter((e) => {
												const r = t.webpackRequire.u(e);
										...
										(t.isReady = !0), a("ready to use"), await i.internalEv.emitAsync("webpack.ready").catch(() => null), window.wppForceMainLoad ? await new Promise((e) => setTimeout(e, 5e3)) : await u;
										for (const e of r)
											try {
												await t.webpackRequire.e(e);
											} catch (r) {
										...
										(t.isFullReady = !0), a("full ready to use"), await i.internalEv.emitAsync("webpack.full_ready").catch(() => null);
									})(e)
								);
						},
					]);
				...
				(t.modules = function (e, r = !1) {
					...
					return a(`${Object.keys(n).length} modules found with: ${null == e ? void 0 : e.toString()}`), n;
				}),
				(t.loadModule = g),
				(t.injectFallbackModule = function (e, r) {
					if (/^fallback_/.test((e += ""))) throw new Error("Invalid fallback ID");
			...
			function g(e) {
				return /^fallback_/.test(e) ? t.fallbackModules[e] : (0, t.webpackRequire)(e);
			}
		},
		1158: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.destroy = async function (e) {
					if (!e || !e.includes("newsletter")) throw new n.WPPError("send_correctly_newsletter_id", "Please, send the correct newsletter ID.");
					try {
		...
		2058: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const n = r(54993);
			(0, n.exportModule)(t, { handleStatusSimpleAck: ["handleStatusSimpleReceipt"], handleStatusSimpleReceipt: ["handleStatusSimpleReceipt"] }, (e) => e.handleStatusSimpleReceipt),
				(0, n.exportModule)(t, { handleChatSimpleAck: ["handleChatSimpleReceipt"], handleChatSimpleReceipt: ["handleChatSimpleReceipt"] }, (e) => e.handleChatSimpleReceipt),
				(0, n.exportModule)(t, { handleGroupSimpleAck: ["handleGroupSimpleReceipt"], handleGroupSimpleReceipt: ["handleGroupSimpleReceipt"] }, (e) => e.handleGroupSimpleReceipt);
		},
		2100: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		2700: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendImageStatus = async function (e, t = {}) {
					const r = new i.MsgKey({ fromMe: !0, id: (0, a.randomHex)(16), participant: i.UserPrefs.getMaybeMeUser(), remote: (0, n.assertWid)("status@broadcast") });
					t = Object.assign(Object.assign(Object.assign({}, s.defaultSendStatusOptions), { messageId: r }), t);
					return await (0, o.sendFileMessage)("status@broadcast", e, Object.assign(Object.assign({}, t), { createChat: !0, type: "image" }));
				});
			const n = r(41687),
				o = r(74023),
				i = r(14647),
		...
		4475: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.addNewLabel = async function (e, t = {}) {
					...
					"string" == typeof r && r.length > 2 && (r = (0, i.getAllLabelColors)().findIndex((e) => e === r));
					if (((r = parseInt(r.toString())), !(await (0, a.colorIsInLabelPalette)(r)))) throw new o.WPPError("color_not_in_pallet", "Color not in pallet");
					const s = await (0, i.getNextLabelId)();
					return await (0, i.labelAddAction)(e, r), await (0, a.getLabelById)(s.toString());
				});
			const n = r(41687),
				o = r(62857),
				i = r(52757),
		...
		4525: (e, t, r) => {
			...
			const n = r(1132),
				...
				s = r(41968),
				u = r(80214);
			(0, o.exportModule)(t, { changeOptInStatusForExternalWebBeta: "changeOptInStatusForExternalWebBeta" }, (e) => e.changeOptInStatusForExternalWebBeta),
				(0, n.injectFallbackModule)("changeOptInStatusForExternalWebBeta", {
					changeOptInStatusForExternalWebBeta: async (e) => {
						await Promise.all([(0, s.setWhatsAppWebExternalBetaDirtyBitIdb)(!0), (0, s.setWhatsAppWebExternalBetaJoinedIdb)(e)]),
							await (0, i.stopComms)(),
							await (0, i.startWebComms)(),
							await (0, i.startHandlingRequests)(),
							await (0, u.syncABPropsTask)(),
							await (0, a.frontendFireAndForget)("changeOptInStatusForExternalWebBeta", {}),
		...
		4749: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				t,
				{ MediaBlobCacheImpl: ["InMemoryMediaBlobCacheImpl", "MediaBlobCacheImpl"], MediaBlobCache: ["InMemoryMediaBlobCache", "MediaBlobCache"] },
		...
		5707: (module, __unused_webpack_exports, __webpack_require__) => {
			...
			const Token = __webpack_require__(44266),
				...
				{ stringToBytes, tarHeaderChecksumMatches, uint32SyncSafeToken } = __webpack_require__(86760),
			...
			async function _fromTokenizer(e) {
				...
				if ((e.fileInfo.size || (e.fileInfo.size = Number.MAX_SAFE_INTEGER), await e.peekBuffer(t, { length: 12, mayBeLess: !0 }), r([66, 77]))) return { ext: "bmp", mime: "image/bmp" };
				...
				if (r([37, 33]))
					return await e.peekBuffer(t, { length: 24, mayBeLess: !0 }), n("PS-Adobe-", { offset: 2 }) && n(" EPSF-", { offset: 14 }) ? { ext: "eps", mime: "application/eps" } : { ext: "ps", mime: "application/postscript" };
				...
				if (r([73, 73, 42, 0]))
					return n("CR", { offset: 8 })
						...
						: r([28, 0, 254, 0], { offset: 8 }) || r([31, 0, 11, 0], { offset: 8 })
							...
							: r([8, 0, 0, 0], { offset: 4 }) && (r([45, 0, 254, 0], { offset: 8 }) || r([39, 0, 254, 0], { offset: 8 }))
								? { ext: "dng", mime: "image/x-adobe-dng" }
								: ((t = Buffer.alloc(24)),
									await e.peekBuffer(t),
									(r([16, 251, 134, 1], { offset: 4 }) || r([8, 0, 0, 0], { offset: 4 })) && r([0, 254, 0, 4, 0, 1, 0, 0, 0, 1, 0, 0, 0, 3, 1], { offset: 9 })
		...
		5724: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { isFilterExcludedFromSearchTreatmentInInboxFlow: "isFilterExcludedFromSearchTreatmentInInboxFlow" }, (e) => e.isFilterExcludedFromSearchTreatmentInInboxFlow);
		},
		5728: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.removeSubgroups = t.promoteParticipants = t.getSubgroups = t.getParticipants = t.getAnnouncementGroup = t.demoteParticipants = t.deactivate = t.create = t.addSubgroups = void 0);
			var n = r(94605);
		...
		6357: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const s = a(r(1132));
			(0, r(54993).exportModule)(t, { fetchLinkPreview: ["getLinkPreview", "default"] }, (e, t) => {
		...
		6678: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { processRawMedia: ["processRawMedia", "default"] }, (e) => {
				var t, r;
				return e.processRawMedia || (null === (r = null === (t = e.default) || void 0 === t ? void 0 : t.toString) || void 0 === r ? void 0 : r.call(t).includes("Received unsupported mediaType"));
			});
		},
		6973: (e, t, r) => {
			"use strict";
		...
		6993: (e, t, r) => {
			...
			async function c(e, t = !0, r = 604800) {
				...
				if (m.isNewsletter) throw new o.WPPError((t ? "pin" : "unpin") + "_error", `The msg ${e.toString()} was not pinned. Not can pin in Newsletter`, { msgId: e, pin: t });
				if (m.isGroup && !(null === (l = null === (c = m.groupMetadata) || void 0 === c ? void 0 : c.participants) || void 0 === l ? void 0 : l.iAmMember()))
					throw new o.WPPError((t ? "pin" : "unpin") + "_error", `You not a member of group, to pin msg ${e.toString()}`, { msgId: e, pin: t });
				if (
					m.isGroup &&
					((null === (d = m.groupMetadata) || void 0 === d ? void 0 : d.restrict) || (null === (f = m.groupMetadata) || void 0 === f ? void 0 : f.announce)) &&
					!(null === (p = m.groupMetadata) || void 0 === p ? void 0 : p.participants.iAmAdmin())
				)
					throw new o.WPPError((t ? "pin" : "unpin") + "_error", `You not have permission to pin msg ${e.toString()}`, { msgId: e, pin: t });
				if (g.isNotification || g.isViewOnce || g.type === a.MSG_TYPE.REVOKED || ((null == g ? void 0 : g.ack) || 0) < a.ACK.SENT)
					throw new o.WPPError((t ? "pin" : "unpin") + "_error", `The msg ${e.toString()} not can be pinned`, { msgId: e, pin: t });
				if (y && y.pinType === (t ? a.PIN_STATE.PIN : a.PIN_STATE.UNPIN)) throw new o.WPPError((t ? "pin" : "unpin") + "_error", `The msg ${e.toString()} is already ${t ? "pinned" : "unpinned"}`, { msgId: e, pin: t });
				const b = await (0, s.sendPinInChatMsg)(g, 1 == t ? a.PIN_STATE.PIN : a.PIN_STATE.UNPIN, t ? r : void 0);
				return { message: g, pinned: b.messageSendResult === a.SendMsgResult.OK ? t : !t, result: b.messageSendResult };
			}
		},
		7071: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.mute = async function (e, t) {
					return await (0, i.ensureNewsletter)(e), !1 === t ? (await (0, o.unmuteNewsletter)([e]), n.NewsletterStore.get(e)) : (await (0, o.muteNewsletter)([e]), n.NewsletterStore.get(e));
				});
			const n = r(14647),
				o = r(52757),
				i = r(16211);
		...
		7103: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.addParticipants = async function (e, t) {
					var r, u;
					const { groupChat: c, participants: l } = await (0, a.ensureGroupAndParticipants)(e, t, !0);
					let d = [];
					d = (0, n.compare)(self.Debug.VERSION, "2.2320.0", ">=")
						? (null === (r = c.groupMetadata) || void 0 === r ? void 0 : r.isLidAddressingMode)
							? l.map((e) => ({ phoneNumber: e.id, lid: i.functions.getCurrentLid(e.id) }))
							: l.map((e) => ({ phoneNumber: e.id }))
						: (null === (u = c.groupMetadata) || void 0 === u ? void 0 : u.isLidAddressingMode)
							? l.map((e) => i.functions.getCurrentLid(e.id))
							: l.map((e) => e.id);
					const f = await i.functions.sendAddParticipants(c.id, d);
					if (f.status >= 400) throw new o.WPPError("group_add_participant_error", "Failed to add participants to the group", { groupId: e, participantsIds: t });
					const p = {};
					for (const e of f.participants || []) {
						let t = null,
							...
							n = null,
							a = null;
						if ("userWid" in e) (t = e.userWid.toString()), (r = e.code), (n = e.invite_code), (a = e.invite_code_exp);
						else {
							t = Object.keys(e)[0];
							const o = e[t];
			...
			const n = r(38385),
				...
				i = r(14647),
				a = r(97944),
				s = { 200: "OK", 403: "Can't join this group because the number was restricted it.", 409: "Can't join this group because the number is already a member of it.", 421: "Member not added, awaiting approval!" };
		},
		7418: (e, t) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		7937: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.blockContact = async function (e) {
					...
					(0, s.compare)(l.SANITIZED_VERSION_STR, "2.2323.4", ">=") ? await d.blockContact({ contact: r, blockEntryPoint: "block_list", bizOptOutArgs: null }) : await d.blockContact(r);
		...
		8489: (e, t, r) => {
			...
			(0, r(54993).exportModule)(t, { StatusModel: ["default"] }, (e) => {
				var t;
				return null === (t = e.default) || void 0 === t ? void 0 : t.prototype.__props.includes("status");
			});
		},
		8686: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.create = async function (e, t) {
					...
					const u = await (0, o.createNewsletterQuery)({ name: e, description: (null == t ? void 0 : t.description) || null, picture: s || null });
					return {
						idJid: null == u ? void 0 : u.idJid,
						inviteCode: null == u ? void 0 : u.newsletterInviteLinkMetadataMixin.inviteCode,
						inviteLink: `https://whatsapp.com/channel/${null == u ? void 0 : u.newsletterInviteLinkMetadataMixin.inviteCode}`,
						name: null === (r = null == u ? void 0 : u.newsletterNameMetadataMixin) || void 0 === r ? void 0 : r.nameElementValue,
						state: null === (i = null == u ? void 0 : u.newsletterStateMetadataMixin) || void 0 === i ? void 0 : i.stateType,
						subscribersCount: null == u ? void 0 : u.newsletterSubscribersMetadataMixin.subscribersCount,
						description: (null == t ? void 0 : t.description) || null,
						timestamp: null === (a = null == u ? void 0 : u.newsletterCreationTimeMetadataMixin) || void 0 === a ? void 0 : a.creationTimeValue,
					};
				});
			const n = r(62857),
				o = r(52757);
		...
		9144: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.reject = async function (e) {
					let t;
					t = e ? o.CallStore.get(e) : o.CallStore.findFirst((e) => e.getState() === i.CALL_STATES.INCOMING_RING || e.isGroup || e.getState() === i.CALL_STATES.ReceivedCall || e.isGroup);
					if (!t) throw new n.WPPError("call_not_found", `Call ${e || "<empty>"} not found`, { callId: e });
					if (t.getState() !== i.CALL_STATES.INCOMING_RING && t.getState() !== i.CALL_STATES.ReceivedCall && !t.isGroup)
						throw new n.WPPError("call_is_not_incoming_ring", `Call ${e || "<empty>"} is not incoming ring`, { callId: e, state: t.getState() });
					t.peerJid.isGroupCall() || (await o.websocket.ensureE2ESessions([t.peerJid]));
					const r = o.websocket.smax("call", { from: o.UserPrefs.getMaybeMeUser().toString({ legacy: !0 }), to: t.peerJid.toString({ legacy: !0 }), id: o.websocket.generateId() }, [
						o.websocket.smax("reject", { "call-id": t.id, "call-creator": t.peerJid.toString({ legacy: !0 }), count: "0" }, null),
					]);
		...
		9534: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { queryGroupInviteCode: "queryGroupInviteCode" }, (e) => e.queryGroupInviteCode);
		},
		9601: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { ReactionsCollection: ["ReactionsCollectionImpl", "ReactionsCollection"] }, (e) => e.ReactionsCollectionImpl || e.ReactionsCollection);
		},
		9793: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.colorIsInLabelPalette = async function (e) {
					...
					return t && (t.includes(e.toString()) || null != t[parseInt(e.toString())]);
				});
			const n = r(41687),
				o = r(62540);
		},
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getAnnouncementGroup = function (e) {
					...
					for (const e of t) {
						...
						if ("LINKED_ANNOUNCEMENT_GROUP" == (null == t ? void 0 : t.groupType)) return t.id;
					}
					return;
				});
			const n = r(14647),
		...
		10964: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.promoteParticipants = async function (e, t) {
					const { groupChat: r, participants: n } = await (0, c.ensureGroupAndParticipants)(e, t);
					if (
						n.some((e) => {
							var t;
		...
		11057: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getHistorySyncProgress = function () {
					...
					return { progress: e.realProgress || 0, paused: ((null == e ? void 0 : e.remainingPausedSeconds) || 0) > 0, inProgress: ((null == e ? void 0 : e.realProgress) || 0) < 100 };
				});
			const s = a(r(52757));
		},
		11102: (e, t, r) => {
		...
		11201: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getCollections = async function (e, t, r) {
					const { collections: i } = await (0, o.queryCollectionsIQ)({ afterCursor: "", catalogWid: (0, n.createWid)(e), height: 100, width: 100, limit: t || 10, productsCount: r || 10 });
					return i;
				});
			const n = r(62857),
				o = r(52757);
		...
		11370: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				...
				(e) =>
					(e.sendAddParticipants && e.sendRemoveParticipants && e.sendPromoteParticipants && e.sendDemoteParticipants) ||
					(e.addGroupParticipants && e.removeGroupParticipants && e.promoteGroupParticipants && e.demoteGroupParticipants)
			);
		},
		11426: (e, t) => {
			"use strict";
		...
		11561: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.createProduct = async function (e) {
					...
					(u.name = e.name.toString()), (u.catalogWid = o.UserPrefs.getMeUser()), (u.imageCdnUrl = s), (u.productImageCollection = new o.ProductImageModel({ mediaUrl: s })), e.description && (u.description = e.description);
					e.price && ((u.priceAmount1000 = 1e4 * e.price), (u.currency = e.currency || "BRL"));
		...
		11782: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { GROUP_JID: "GROUP_JID", CHAT_JID: "CHAT_JID" }, (e) => e.GROUP_JID && e.CHAT_JID);
		},
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.resizeImage = function (e, t = {}) {
					return new Promise((r, n) => {
						new o.default(e, Object.assign(Object.assign({}, t), { success: r, error: n }));
					});
				});
			const o = n(r(23807));
		},
		...
		13257: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.createWid = function (e) {
					...
					if (/@\w*lid\b/.test(e)) return n.WidFactory.createUserWid(e, "lid");
					if (/^\d+$/.test(e)) return n.WidFactory.createUserWid(e, "c.us");
					if (/^\d+-\d+$/.test(e)) return n.WidFactory.createUserWid(e, "g.us");
					if (/status$/.test(e)) return n.WidFactory.createUserWid(e, "broadcast");
		...
		14647: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }), (t.websocket = t.multidevice = t.functions = t._moduleIdMap = t.enums = t.contants = void 0), a(r(12105), t), (t.contants = s(r(19198))), (t.enums = s(r(20514)));
			var u = r(54993);
		...
		15035: (e, t) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.encryptAndParserMsgButtons = async function (e, t, r, n, o, i, a) {
					...
					const c = [];
					if (null === (u = null === (s = null == t ? void 0 : t.viewOnceMessage) || void 0 === s ? void 0 : s.message) || void 0 === u ? void 0 : u.interactiveMessage) {
						const s = (function (e, t) {
							var r, n, o, i, a, s, u, c, l;
							const d = t.filter((e) => !e.device),
								f = t.filter((e) => e.device),
								p = null === (n = null === (r = e.viewOnceMessage) || void 0 === r ? void 0 : r.message) || void 0 === n ? void 0 : n.interactiveMessage;
							let g = !1;
							const m = JSON.parse(JSON.stringify(e));
							if (p) {
								const t = ["documentMessage", "documentWithCaptionMessage", "imageMessage", "videoMessage"];
								let r,
									n = 1;
								for (let e of t)
									if (e in p.header) {
										const t = e;
										"documentWithCaptionMessage" === e && (e = "documentMessage"), (r = { [t]: p.header[t] }), (n = "imageMessage" == t ? 4 : t.includes("document") ? 3 : "videoMessage" == t ? 5 : 1);
										break;
									}
								const d = {
									message: {
										buttonsMessage: Object.assign(
											Object.assign(
												{
													headerType: n,
													contentText: (null === (o = null == p ? void 0 : p.body) || void 0 === o ? void 0 : o.text) || " ",
													footerText: (null === (i = null == p ? void 0 : p.footer) || void 0 === i ? void 0 : i.text) || " ",
												},
												r
											),
											{
												buttons:
													null === (a = null == p ? void 0 : p.nativeFlowMessage) || void 0 === a
														? void 0
														: a.buttons
															.map((e, t) => {
																var r, n;
																return "quick_reply" == e.name
																	? {
																		type: 1,
																		buttonId: (null === (r = JSON.parse(e.buttonParamsJson)) || void 0 === r ? void 0 : r.id) || `${t}`,
																		buttonText: { displayText: (null === (n = JSON.parse(e.buttonParamsJson)) || void 0 === n ? void 0 : n.display_text) || " " },
																	}
																	: ((g = !0), null);
															})
															.filter((e) => null != e),
												...
												hydratedTemplate: Object.assign(
													Object.assign(
														Object.assign(
															{
																hydratedButtons:
																	null === (s = null == p ? void 0 : p.nativeFlowMessage) || void 0 === s
																		? void 0
																		: s.buttons
																			.map((e, t) => {
																				var r, n, o, i, a, s, u, c;
																				return "quick_reply" == e.name
																					? {
																						index: t,
																						quickReplyButton: {
																							displayText: (null === (r = JSON.parse(e.buttonParamsJson)) || void 0 === r ? void 0 : r.display_text) || " ",
																							id: (null === (n = JSON.parse(e.buttonParamsJson)) || void 0 === n ? void 0 : n.id) || `${t}`,
																						},
																					}
																					: "cta_url" == e.name
																						? {
																							index: t,
																							urlButton: {
																								displayText: (null === (o = JSON.parse(e.buttonParamsJson)) || void 0 === o ? void 0 : o.display_text) || " ",
																								url: null === (i = JSON.parse(e.buttonParamsJson)) || void 0 === i ? void 0 : i.url,
																							},
																						}
																						: "cta_copy" == e.name
																							? {
																								index: t,
																								urlButton: {
																									displayText: (null === (a = JSON.parse(e.buttonParamsJson)) || void 0 === a ? void 0 : a.display_text) || " ",
																							...
																							: "cta_call" == e.name
																								? {
																									index: t,
																									callButton: {
																										displayText: (null === (u = JSON.parse(e.buttonParamsJson)) || void 0 === u ? void 0 : u.display_text) || " ",
																										phoneNumber: null === (c = JSON.parse(e.buttonParamsJson)) || void 0 === c ? void 0 : c.phone_number,
																									},
																								}
																								: null;
																			})
														...
														1 == n ? { hydratedTitleText: (null === (u = p.header) || void 0 === u ? void 0 : u.title) || " " } : void 0
													),
													{
														hydratedContentText: (null === (c = null == p ? void 0 : p.body) || void 0 === c ? void 0 : c.text) || " ",
														hydratedFooterText: (null === (l = null == p ? void 0 : p.footer) || void 0 === l ? void 0 : l.text) || " ",
													}
												),
											},
										},
									};
								delete m.viewOnceMessage, (m.documentWithCaptionMessage = g ? f : d), (m.messageContextInfo = e.messageContextInfo);
							}
							return [
								{ proto: e, devices: d },
								{ proto: m, devices: f },
						...
						s.map(async (t) => {
							const r = await a(e, t.proto, t.devices, n, o, "function" != typeof i ? i : void 0);
							c.push(...r.stanza.content[0].content);
						});
					}
					const l = await a(e, t, r, n, o, "function" != typeof i ? i : void 0);
		...
		15878: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { createChatRecord: "createChatRecord" }, (e) => e.createChatRecord);
		},
		15952: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.prepareLinkPreview = async function (e, t) {
					...
					"object" == typeof t.linkPreview && ((e.subtype = "url"), (e = Object.assign(Object.assign({}, e), t.linkPreview)));
					return e;
				});
			const u = r(73113),
				c = a(r(1132)),
						...
						return new Promise(async (r) => {
							try {
								...
								const { imageUrl: t } = e,
									o = s(e, ["imageUrl"]);
								let i = {};
								t && (i = await (0, u.generateThumbnailLinkPreviewData)(t).catch(() => null));
		...
		16211: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.ensureNewsletter = async function (e) {
					const t = (0, n.assertGetChat)(e);
					if (!t.isNewsletter) throw new o.WPPError("not_a_newsletter", `Chat ${t.id._serialized} is not a newsletter`);
					return t;
				});
			const n = r(41687),
				o = r(62857);
				...
				(function () {
					c.MsgStore.on("add", (e) => {
								...
								"ciphertext" ===
									(e = await (async function (e) {
										if (void 0 !== e.quotedStanzaID) {
											...
											if (!t) return e;
											Object.defineProperties(e, { _quotedMsgObj: { value: t, writable: !1 }, quotedMsgId: { value: t.id, writable: !1 } });
										}
										return e;
									})(e)).type &&
									e.once("change:type", () => {
						...
						Object.defineProperty(c.MsgModel.prototype, "chat", {
							get: function () {
								var e;
								return c.ChatStore.get((null === (e = this.id) || void 0 === e ? void 0 : e.fromMe) ? this.to : this.from);
							},
							configurable: !0,
						});
		...
		17459: function (e, t, r) {
			...
			u.onInjected(function () {
				let e = !1;
				const t = async () => {
					(0, d.isAuthenticated)() ? (e = !1) : e || (c.Socket.state !== l.SOCKET_STATE.UNPAIRED && c.Socket.state !== l.SOCKET_STATE.UNPAIRED_IDLE) || ((e = !0), s.internalEv.emit("conn.require_auth"));
				};
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getMessageById = async function (e) {
					...
					for (const e of d) {
						...
						const d =
							"function" == typeof (null === (t = e.remote) || void 0 === t ? void 0 : t.isStatusV3)
								? null === (r = e.remote) || void 0 === r
									? void 0
									: r.isStatusV3()
								: null === (o = null === (n = e.remote) || void 0 === n ? void 0 : n.toString()) || void 0 === o
									? void 0
						...
						if (!l) throw (c(`message id ${e.toString()} not found`), new a.WPPError("msg_not_found", `Message ${e.toString()} not found`, { id: e.toString() }));
						f.push(l);
					}
					if (((f = f.map((e) => (e instanceof s.MsgModel ? e : s.MsgStore.get(e) || new s.MsgModel(e)))), l)) return f[0];
					return f;
				});
			const o = n(r(17833)),
				i = r(41687),
		...
		18194: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setInputText = async function (e, t) {
					const r = void 0 !== t ? await (0, n.assertFindChat)(t) : (0, o.getActiveChat)();
					if (r)
						return (null == r ? void 0 : r.active)
							? (r.setComposeContents({ text: e, timestamp: (0, s.unixTime)() }), a.ComposeBoxActions.setTextContent(r, e), r.getComposeContents())
							: (null == r || r.setComposeContents({ text: e, timestamp: (0, s.unixTime)() }), r.getComposeContents());
		...
		18795: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { BotProfileCollection: ["BotProfileCollection"] }, (e) => e.BotProfileCollection);
		},
		18827: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		19170: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, "ApiContact", (e) => e.getCurrentLid && e.getPhoneNumber);
		},
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.prepareRawMessage = async function (e, t, r = {}) {
					...
					(r = Object.assign(Object.assign({}, c.defaultSendMessageOptions), r)),
						(t = Object.assign({ t: (0, u.unixTime)(), from: a.UserPrefs.getMaybeMeUser(), to: e.id, self: "out", isNewMsg: !0, local: !0, ack: s.ACK.CLOCK }, t)),
						r.delay &&
						("chat" == t.type ? await (0, l.markIsComposing)(e.id) : (null == r ? void 0 : r.isPtt) && (await (0, l.markIsRecording)(e.id)),
					...
					(null === (d = e.id) || void 0 === d ? void 0 : d.isBot()) &&
						(t = Object.assign(Object.assign({}, t), {
							messageSecret: await (0, u.genBotMsgSecretFromMsgSecret)(crypto.getRandomValues(new Uint8Array(32))),
							botPersonaId: a.BotProfileStore.get(null === (f = e.id) || void 0 === f ? void 0 : f.toString()).personaId,
						}));
					if (r.messageId) {
						...
						if (!r.messageId.remote.equals(e.id)) throw new i.WPPError("message_key_remote_id_is_not_same_of_chat", "Message key remote ID is not same of chat", { messageId: r.messageId.toString() });
						t.id = r.messageId;
					}
					t.id || (t.id = await (0, l.generateMessageID)(e));
					if (r.mentionedList && !Array.isArray(r.mentionedList)) throw new i.WPPError("mentioned_list_is_not_array", "The option mentionedList is not an array", { mentionedList: r.mentionedList });
					...
					if (r.mentionedList) {
						...
						for (const t of e) if (!t.isUser()) throw new i.WPPError("mentioned_is_not_user", "Mentioned is not an user", { mentionedId: t.toString() });
						t.mentionedJidList = e;
					}
					if (r.quotedMsg) {
						if (
							("string" == typeof r.quotedMsg && (r.quotedMsg = a.MsgKey.fromString(r.quotedMsg)),
								r.quotedMsg instanceof a.MsgKey && (r.quotedMsg = await (0, l.getMessageById)(r.quotedMsg)),
								!(r.quotedMsg instanceof a.MsgModel))
						)
							throw new i.WPPError("invalid_quoted_msg", "Invalid quotedMsg", { quotedMsg: r.quotedMsg });
						if (!(null === (p = r.quotedMsg) || void 0 === p ? void 0 : p.isStatusV3) && !(0, u.canReplyMsg)(r.quotedMsg))
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setMyProfilePicture = async function (e) {
					const t = await (0, n.convertToFile)(e),
						r = await (0, n.resizeImage)(t, { width: 96, height: 96, mimeType: "image/jpeg", resize: "cover" }),
						a = await (0, n.resizeImage)(t, { width: 640, height: 640, mimeType: "image/jpeg", resize: "cover" }),
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.create = async function (e, t, r) {
					...
					for (const e of o) {
						...
						if (!r) throw new l.WPPError("participant_not_exists", "Participant not exists", { id: e });
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getReactions = async function (e) {
					...
					for (const e of t.reactions) {
						...
						for (const r of e.senders) t.senders.push({ id: r.msgKey, msgId: r.parentMsgKey, reactionText: r.reactionText, read: r.read, sender: (0, n.createWid)(r.senderUserJid), orphan: r.orphan, timestamp: r.timestamp });
						r.push(t);
					}
					return {
		...
		20748: (e, t) => {
			...
			function r(e, t, r, n) {
				var o = { timer: void 0, lastArgs: [] },
					i = function () {
						for (var n = this, i = [], a = 0; a < arguments.length; a++) i[a] = arguments[a];
						(o.lastArgs = i),
							o.timer ? clearTimeout(o.timer) : t && r.apply(this, o.lastArgs),
			...
			function n(e, t) {
				...
				var i = n[0],
					a = n[1],
					s = 3 === n.length && n[2] ? n[2] : Object.getOwnPropertyDescriptor(i, a);
				if (s)
		...
		20941: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.unsubscribePresence = t.subscribePresence = t.queryExists = t.list = t.getStatus = t.getProfilePictureUrl = t.getCommonGroups = t.getBusinessProfile = t.get = void 0);
			var n = r(93037);
			Object.defineProperty(t, "get", {
				enumerable: !0,
				get: function () {
		...
		21139: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.accept = async function (e) {
					let t;
					t = e ? o.CallStore.get(e) : o.CallStore.findFirst((e) => e.getState() === i.CALL_STATES.INCOMING_RING || e.isGroup);
					if (!t) throw new n.WPPError("call_not_found", `Call ${e || "<empty>"} not found`, { callId: e });
					if ("INCOMING_RING" !== t.getState() && !t.isGroup) throw new n.WPPError("call_is_not_incoming_ring", `Call ${e || "<empty>"} is not incoming ring`, { callId: e, state: t.getState() });
					t.peerJid.isGroupCall() || (await o.websocket.ensureE2ESessions([t.peerJid]));
					const r = [o.websocket.smax("audio", { enc: "opus", rate: "16000" }, null), o.websocket.smax("audio", { enc: "opus", rate: "8000" }, null)];
					t.isVideo && r.push(o.websocket.smax("video", { orientation: "0", screen_width: "1920", screen_height: "1080", device_orientation: "0", enc: "vp8", dec: "vp8" }, null));
					r.push(o.websocket.smax("net", { medium: "3" }, null), o.websocket.smax("encopt", { keygen: "2" }, null));
					const a = o.websocket.smax("call", { to: t.peerJid.toString({ legacy: !0 }), id: o.websocket.generateId() }, [o.websocket.smax("accept", { "call-id": t.id, "call-creator": t.peerJid.toString({ legacy: !0 }) }, r)]);
		...
		21268: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getPastParticipants = async function (e) {
					return (await (0, n.ensureGroup)(e)).groupMetadata.pastParticipants.getModelsArray();
				});
			const n = r(2818);
		},
		21330: (e, t, r) => {
		...
		21892: (e, t, r) => {
			...
			const n = r(64456),
				...
				i = r(54993),
				a = r(9534);
			(0, i.exportModule)(t, { fetchMexGroupInviteCode: "fetchMexGroupInviteCode" }, (e) => e.fetchMexGroupInviteCode),
				(0, o.injectFallbackModule)("fetchMexGroupInviteCode", {
					fetchMexGroupInviteCode: async (e) => {
						const t = await (0, n.iAmAdmin)(e);
						return await (0, a.queryGroupInviteCode)(e, t).then((e) => e.code);
					},
				});
		},
		...
		22418: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.trackException = function (e, t = !1) {
					...
					p.trackEvent("exception", { description: e, fatal: t }), g && g.trackEvent("exception", { description: e, fatal: t });
				});
			const u = r(91491),
				c = a(r(72927)),
				l = r(13691),
			...
			const f = ["W: ", "-", ", WA-JS: ", t.waVersion],
				...
				g = u.config.googleAnalyticsId ? new d.Tracker(u.config.googleAnalyticsId) : null;
			l.internalEv.on("webpack.injected", () => {
				const e = c.isAuthenticated(),
					r = c.isMultiDevice() ? "multidevice" : "legacy";
				if (
					(u.config.disableGoogleAnalytics || ((p.documentTitle = f.join("")), p.setUserProperty("method", r), p.setUserProperty("wa_js", t.waVersion), p.setUserProperty("powered_by", u.config.poweredBy || "-")),
						l.internalEv.on("conn.main_init", () => {
							var e;
							(f[1] = (null === (e = window.Debug) || void 0 === e ? void 0 : e.VERSION) || "-"), u.config.disableGoogleAnalytics || ((p.documentTitle = f.join("")), p.setUserProperty("whatsapp", f[1]));
						}),
					...
					if (
						(p.trackEvent("page_view", { authenticated: e, method: r }),
							...
							l.internalEv.on("conn.main_init", () => {
								var e;
								(f[1] = (null === (e = window.Debug) || void 0 === e ? void 0 : e.VERSION) || "-"), (g.documentTitle = f.join("")), g.setUserProperty("whatsapp", f[1]);
							}),
				...
				l.internalEv.on("config.update", (e) => {
					if (!u.config.disableGoogleAnalytics)
						if ("poweredBy" === e.path[0]) p.setUserProperty("powered_by", e.value || "-"), g && g.setUserProperty("powered_by", e.value || "-");
						else if ("googleAnalyticsUserProperty" === e.path[0] && g && "object" == typeof u.config.googleAnalyticsUserProperty)
							for (const e in u.config.googleAnalyticsUserProperty) {
								const t = u.config.googleAnalyticsUserProperty[e];
								g.setUserProperty(e, t);
							}
				...
				(l.internalEv.on("conn.authenticated", () => {
					...
					p.trackEvent("login", { method: e }), g && p.trackEvent("login", { method: e });
				}),
					l.internalEv.on("conn.logout", () => {
						const e = c.isMultiDevice() ? "multidevice" : "legacy";
						p.trackEvent("logout", { method: e }), g && g.trackEvent("logout", { method: e });
					}));
		},
		22523: (e, t, r) => {
			"use strict";
		...
		22696: (e, t, r) => {
			...
			const n = r(1132),
				...
				i = r(44804),
				a = r(42547);
			(0, o.exportModule)(t, { getVotes: "getVotes", getVote: "getVote" }, (e) => e.getVotes && e.getVote),
		...
		22987: (e, t) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setKeepAlive = function (e = !0) {
					e ? ((document.hasFocus = () => !0), (n = setInterval(() => document.dispatchEvent(new Event("scroll")), 15e3))) : ((document.hasFocus = r), n && (clearInterval(n), (n = null)));
					return !!n;
				});
		...
		23144: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setMyStatus = t.setMyProfilePicture = t.setMyProfileName = t.removeMyProfilePicture = t.isBusiness = t.getMyStatus = t.getMyProfilePicture = t.getMyProfileName = t.editBusinessProfile = void 0);
			var n = r(40923);
		...
		23527: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.ensureGroup = async function (e, t = !1) {
					...
					if (t && !a.participants.iAmAdmin() && "all_member_add" !== a.memberAddMode) throw new o.WPPError("group_you_are_not_admin", `You are not admin in ${r.id._serialized}`);
					return r;
				});
			const n = r(41687),
				o = r(62857),
		...
		23572: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				t,
				{ StatusV3Collection: ["StatusV3CollectionImpl", "StatusCollectionImpl", "StatusCollection"] },
				(e) => e.StatusV3CollectionImpl || e.StatusV3Collection || e.StatusCollectionImpl || e.StatusCollection
			);
		},
		23749: (e, t, r) => {
			"use strict";
		...
		23752: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { ServerPropsModel: "ServerProps" }, (e) => (e.getMaxFilesSizeServerProp && e.ServerProps) || (e.getMaxFilesSize && e.ServerProps));
		},
		23807: function (e) {
			e.exports = (function () {
				"use strict";
								...
								s =
									...
									function (e) {
										...
										for (
											...
											d[f] = c.charCodeAt(f);
										return n ? new Blob([o ? d : l], { type: r }) : ((p = new i()).append(l), p.getBlob(r));
									};
							t.HTMLCanvasElement &&
								!r.toBlob &&
								(r.mozGetAsFile
									? (r.toBlob = function (e, t, n) {
										var o = this;
										setTimeout(function () {
											n && r.toDataURL && s ? e(s(o.toDataURL(t, n))) : e(o.mozGetAsFile("blob", t));
										});
									})
									: r.toDataURL &&
									s &&
									(r.msToBlob
										? (r.toBlob = function (e, t, n) {
											var o = this;
											setTimeout(function () {
												((t && "image/png" !== t) || n) && r.toDataURL && s ? e(s(o.toDataURL(t, n))) : e(o.msToBlob(t));
											});
										})
				...
				var l = c.exports,
					...
					p = "undefined" != typeof window && void 0 !== window.document ? window : {},
					g = function (e) {
						return e > 0 && e < 1 / 0;
					},
				...
				var M = p.btoa;
				function w(e, t) {
					for (var r = [], n = 8192, o = new Uint8Array(e); o.length > 0;) r.push(_.apply(null, y(o.subarray(0, n)))), (o = o.subarray(n));
				...
				function I(e) {
					for (var t = y(new Uint8Array(e)), r = t.length, n = [], o = 0; o + 3 < r;) {
						var i = t[o],
							a = t[o + 1];
				...
				return (function () {
					function e(n, o) {
						r(this, e), (this.file = n), (this.exif = []), (this.image = new Image()), (this.options = t(t({}, f), o)), (this.aborted = !1), (this.result = null), this.init();
					}
					return (
						o(
							e,
							[
														...
														(this.reader = u),
															(u.onload = function (r) {
																var o = r.target.result,
																	u = {},
																	c = 1;
																i && (c = O(o)) > 1 && a(u, j(c)), s && (e.exif = I(o)), (u.url = i || s ? (!k || c > 1 ? w(o, n) : k.createObjectURL(t)) : o), e.load(u);
															}),
															(u.onabort = function () {
															...
															i || s ? u.readAsArrayBuffer(t) : u.readAsDataURL(t);
								...
								},
								{
									...
									value: function (e) {
										...
										(o.onload = function () {
											...
											p.navigator && /(?:iPad|iPhone|iPod).*?AppleWebKit/i.test(p.navigator.userAgent) && (o.crossOrigin = "anonymous"),
											(o.alt = n.name),
											(o.src = e.url);
									},
								},
								{
									...
									value: function (e) {
										...
										v && (j = S / I);
										var L = x({ aspectRatio: j, width: _, height: P }, "contain");
										(_ = L.width), (P = L.height);
										var R = x({ aspectRatio: j, width: M, height: O }, "cover");
										if (((M = R.width), (O = R.height), v)) {
											var N = x({ aspectRatio: j, width: S, height: I }, p.resize);
										...
										var W = -(S = Math.floor(C(Math.min(Math.max(S, M), _)))) / 2,
											G = -(I = Math.floor(C(Math.min(Math.max(I, O), P)))) / 2,
											q = S,
											z = I,
											$ = [];
										if (v) {
											var V = 0,
												...
												J = r,
												H = n,
												Q = x({ aspectRatio: j, width: r, height: n }, { contain: "cover", cover: "contain" }[p.resize]);
											(J = Q.width), (H = Q.height), (V = (r - J) / 2), (K = (n - H) / 2), $.push(V, K, J, H);
										}
										if (($.push(W, G, q, z), b)) {
											var Y = [I, S];
											(S = Y[0]), (I = Y[1]);
										}
										(m.width = S), (m.height = I), h(p.mimeType) || (p.mimeType = d.type);
										var Z = "transparent";
										d.size > p.convertSize && p.convertTypes.indexOf(p.mimeType) >= 0 && (p.mimeType = "image/jpeg");
										...
										if (
											(X && (Z = "#fff"),
												...
												!this.aborted &&
												(y.save(), y.translate(S / 2, I / 2), y.rotate((i * Math.PI) / 180), y.scale(s, c), y.drawImage.apply(y, [f].concat($)), y.restore(), p.drew && p.drew.call(this, y, m), !this.aborted))
										) {
											var ee = function (e) {
												if (!t.aborted) {
													var o = function (e) {
								...
								},
								{
									...
									value: function (e) {
										var t = e.naturalWidth,
											...
											i = this.image,
											a = this.options;
										if ((k && 0 === i.src.indexOf("blob:") && k.revokeObjectURL(i.src), n))
											if (a.strict && !a.retainExif && n.size > o.size && a.mimeType === o.type && !(a.width > t || a.height > r || a.minWidth > t || a.minHeight > r || a.maxWidth < t || a.maxHeight < r)) n = o;
											else {
												var s = new Date();
								...
								},
								{
									...
									value: function () {
										this.aborted ||
											((this.aborted = !0),
												this.reader ? this.reader.abort() : this.image.complete ? this.fail(new Error("The compression process has been aborted.")) : ((this.image.onload = null), this.image.onabort()));
									},
								},
							],
							[
						...
						(o.crossOrigin = "anonymous"),
							...
							(o.onload = function () {
								...
								const i = r.getImageData(0, 0, e.width, e.height).data,
									...
									s = (a[1] << 56) + (a[2] << 48) + (a[3] << 40) + (a[4] << 32) + (a[5] << 24) + (a[6] << 16) + (a[7] << 8) + a[8];
		...
		24393: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.submit = async function (e, t, r) {
					var l, d, f;
					if (e.toString() == (null === (l = (0, i.getMyUserId)()) || void 0 === l ? void 0 : l.toString())) throw new a.WPPError("can_not_submit_order_to_yourself", "You can not submit order to yourself");
					const p = s.CartStore.findCart(e);
					if (!p || !(null === (d = null == p ? void 0 : p.cartItemCollection) || void 0 === d ? void 0 : d.length)) throw new a.WPPError("cart_not_have_products", `Cart from  ${e || "<empty>"} not have products`, { wid: e });
					const g = await (0, n.assertFindChat)((0, a.createWid)(e)),
						m = await (0, u.createOrder)(g.id, p.cartItemCollection.toArray()),
						...
						b = await (0, o.prepareRawMessage)(
							g,
							{
								...
								totalCurrencyCode: m.price.currency && m.price.currency.length > 0 ? m.price.currency : 0,
							},
							r
						);
					if ((await (0, u.addAndSendMsgToChat)(g, b), (0, u.updateCart)(p), await (0, c.clear)(e), !m.id)) throw new a.WPPError("error_send_order_request", "Error when sending order request");
		...
		24444: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { genBotMsgSecretFromMsgSecret: "genBotMsgSecretFromMsgSecret" }, (e) => e.genBotMsgSecretFromMsgSecret);
		},
		24654: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendFileMessage = async function (e, t, r) {
					var n, o, i;
					let a = (r = Object.assign(Object.assign(Object.assign({}, b.defaultSendMessageOptions), { type: "auto-detect", waveform: !0 }), r)).createChat ? await (0, c.assertFindChat)(e) : (0, c.assertGetChat)(e);
					"status@broadcast" == (null == e ? void 0 : e.toString()) && (a = new p.ChatModel({ id: (0, l.createWid)(y.STATUS_JID) }));
					...
					let P, M;
					"audio" === r.type
						? ((m.isPtt = r.isPtt), r.isPtt && (P = r.isViewOnce), (m.precomputedFields = await (0, v.prepareAudioWaveform)(r, s)))
						: "image" === r.type
							? ((P = r.isViewOnce), (M = (null == r ? void 0 : r.isHD) ? 2560 : 1600))
							: "video" === r.type
								? ((P = r.isViewOnce), (m.asGif = r.isGif))
								: "document" === r.type
									? (m.asDocument = !0)
									: "sticker" === r.type && (m.asSticker = !0);
					const w = p.MediaPrep.prepRawMedia(f, Object.assign(Object.assign({}, m), { maxDimension: M }));
					let O = await (0, h.prepareRawMessage)(a, { caption: r.caption || u, filename: u, footer: r.footer, isCaptionByUser: null != r.caption }, r);
					(O = (0, h.prepareMessageButtons)(O, r)), r.markIsRead && (_("marking chat is read before send file"), await (0, h.markIsRead)(a.id).catch(() => null));
					await w.waitForPrep(), (null == r ? void 0 : r.isPtv) && ((w._mediaData.type = "ptv"), (w._mediaData.fullHeight = 1128), (w._mediaData.fullWidth = 1128));
					_(`sending message (${r.type}) with id ${O.id}`);
					const j = w.sendToChat(a, { caption: r.caption, footer: r.footer, isViewOnce: P, productMsgOptions: "status@broadcast" === e ? void 0 : O, addEvenWhilePreparing: !1, type: O.type });
					let S = null;
					S =
						"status@broadcast" == (null === (n = O.to) || void 0 === n ? void 0 : n.toString())
							? await new Promise((e) => {
								p.StatusV3Store.on("change:lastReceivedKey", async function t(r, n) {
									var o;
									if (r.id.toString() == (null === (o = O.from) || void 0 === o ? void 0 : o.toString())) {
					...
					if (
						...
						return { id: null === (o = S.id) || void 0 === o ? void 0 : o.toString(), ack: S.ack, sendMsgResult: j };
					}
					{
						const e = await new Promise((e, t) => {
							const r = setTimeout(() => {
								t(new l.WPPError("timeout_on_send_status", "Timeout for wait response of send media status"));
							}, 3e4),
								n = setInterval(async () => {
						...
						return { id: null === (i = e.id) || void 0 === i ? void 0 : i.toString(), ack: e.ack, sendMsgResult: { messageSendResult: g.SendMsgResult.OK } };
					}
				});
			const u = s(r(17833)),
				c = r(41687),
			...
			f.onFullReady(() => {
				(0, m.wrapModuleFunction)(y.generateVideoThumbsAndDuration, async (e, ...t) => {
					const [r] = t;
					try {
						return await e(...t);
					} catch (e) {
								...
								return {
									...
									thumbs: r.maxDimensions.map((e) =>
										(function (e, t, r) {
											...
											const i = { width: Math.max(n, 1), height: Math.max(o, 1) },
											...
											return (
												...
												{ url: a.toDataURL("image/jpeg"), width: i.width, height: i.height, fullWidth: e, fullHeight: t }
											);
										})(t.width, t.height, e)
									),
								};
					...
					(0, m.wrapModuleFunction)(y.processRawSticker, async (e, ...t) => {
						...
						if ("image/webp" === r.type()) {
							...
							(0, y.isAnimatedWebp)(t) && (n.mediaBlob = await p.OpaqueData.createFromData(e, r.type()));
						}
						return n;
					}),
					(0, m.wrapModuleFunction)(y.uploadMedia, async (e, ...t) => {
						const [r] = t;
						return "ptv" == r.mediaType ? ((r.mediaType = "video"), await e(r)) : await e(...t);
					});
			});
		},
		24846: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.edit = async function (e, t) {
					...
					await (0, o.editNewsletterMetadataAction)(
						...
						{ editDescription: !!t.description, editName: !!t.name, editPicture: !(!l && null != t.picture) },
						{ name: t.name, description: t.description, picture: null == t.picture ? null : l }
					);
					const d = await (0, o.queryNewsletterMetadataByJid)(e, { picture: !0, description: !0, name: !0, state: !0, creationTime: !0 });
					return {
						idJid: null == d ? void 0 : d.idJid,
						inviteCode: null == d ? void 0 : d.newsletterInviteLinkMetadataMixin.inviteCode,
						inviteLink: `https://whatsapp.com/channel/${null == d ? void 0 : d.newsletterInviteLinkMetadataMixin.inviteCode}`,
						name: null === (r = null == d ? void 0 : d.newsletterNameMetadataMixin) || void 0 === r ? void 0 : r.nameElementValue,
						state: null === (a = null == d ? void 0 : d.newsletterStateMetadataMixin) || void 0 === a ? void 0 : a.stateType,
						subscribersCount: null == d ? void 0 : d.newsletterSubscribersMetadataMixin.subscribersCount,
						description:
							null === (u = null === (s = null == d ? void 0 : d.newsletterDescriptionMetadataMixin) || void 0 === s ? void 0 : s.descriptionQueryDescriptionResponseMixin) || void 0 === u ? void 0 : u.elementValue,
						timestamp: null === (c = null == d ? void 0 : d.newsletterCreationTimeMetadataMixin) || void 0 === c ? void 0 : c.creationTimeValue,
					};
				});
			const n = r(62857),
				o = r(52757),
		...
		25736: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const n = r(54993);
			(0, n.exportModule)(t, { addProductToCart: "addProductToCart" }, (e) => e.addProductToCart),
				(0, n.exportModule)(t, { updateProductQuantityCart: "default" }, (e) => {
					var t, r;
					return null === (r = null === (t = e.default) || void 0 === t ? void 0 : t.displayName) || void 0 === r ? void 0 : r.includes("BizUpdateProductQuantityCartAction");
				}),
				(0, n.exportModule)(t, { deleteProductFromCart: "default" }, (e) => {
					var t, r;
					return null === (r = null === (t = e.default) || void 0 === t ? void 0 : t.displayName) || void 0 === r ? void 0 : r.includes("BizDeleteProductFromCartAction");
				});
		},
		25882: (e, t, r) => {
			"use strict";
		...
		26947: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setAddGroup = async function (e, t) {
					if ("string" != typeof e || !Object.values(s).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set add group privacy`, { value: e });
					const r = await (0, a.prepareDisallowedList)(n.PrivacyDisallowedListType.GroupAdd, e, t);
					return (
						await (0, i.setPrivacyForOneCategory)(
							{ name: n.PrivacyDisallowedListType.GroupAdd, value: e, dhash: "contact_blacklist" === e ? r.dhash : null, users: "contact_blacklist" === e ? r.idsFormatted : void 0 },
			...
			var s;
			!(function (e) {
				(e.all = "all"), (e.contacts = "contacts"), (e.contact_blacklist = "contact_blacklist");
		...
		27112: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.mute = async function (e, t) {
					...
					let s = 0;
					if ("expiration" in t) s = "number" == typeof t.expiration ? t.expiration : t.expiration.getTime() / 1e3;
					else {
						if (!("duration" in t)) throw new o.WPPError("invalid_time_mute", "Invalid time for mute", { time: t });
						s = (0, i.unixTime)() + t.duration;
					}
					if (s < (0, i.unixTime)()) throw new o.WPPError("negative_time_mute", "Negative duration for mute", { time: t });
					return await a.mute.mute({ expiration: s, isAutoMuted: !1, sendDevice: !0 }), { wid: r, expiration: a.mute.expiration, isMuted: 0 !== a.mute.expiration };
				});
			const n = r(41687),
				o = r(62857),
				i = r(52757);
		...
		27573: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.refreshQR = async function () {
					...
					return await n.internalEv.waitFor("conn.auth_code_change").then((e) => e[0]);
				});
			const n = r(13691),
				o = r(14647),
				i = r(60397);
		...
		28068: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getLabelById = async function (e) {
					...
					return { id: t.id, name: t.name, color: t.hexColor ? (0, n.assertColor)(t.hexColor) : null, count: (0, s.patchLabelCount)(t), hexColor: (0, a.colorIndexToHex)(t.colorIndex), colorIndex: t.colorIndex };
				});
			const n = r(41687),
				o = r(62857),
				i = r(14647),
		...
		28081: (e, t, r) => {
			...
			(0, r(54993).exportModule)(t, { revokeStatus: ["default"] }, (e) => {
				var t, r;
				return null === (r = null === (t = e.default) || void 0 === t ? void 0 : t.displayName) || void 0 === r ? void 0 : r.includes("RevokeStatusAction");
			});
		},
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.postSendStatus = function (e) {
					e.sendMsgResult.then(async () => {
						...
						if (!t) return;
						n.StatusV3Store.addStatusMessages(t.author, [t]), n.StatusV3Store.handleUpdate(t.attributes, null, !1);
		...
		28690: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { isAuthenticated: ["isLoggedIn", "Z"], isLoggedIn: ["isLoggedIn", "Z"] }, (e) => {
				var t, r;
		...
		28904: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.genLinkDeviceCodeForPhoneNumber = async function (e, t = !0) {
					if (!e || "string" != typeof e) throw new n.WPPError("send_the_phone_number_to_connect", "Can't get code for without phone number param");
					...
					return await o.functions.initializeAltDeviceLinking(), await o.functions.genLinkDeviceCodeForPhoneNumber(e, t);
				});
			const n = r(62857),
				o = r(14647),
				i = r(60397);
		...
		28921: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.markIsComposing = async function (e, t) {
					const r = (0, n.assertGetChat)(e);
					await r.presence.subscribe(), await o.ChatPresence.markComposing(r), r.pausedTimerId && (clearTimeout(r.pausedTimerId), r.unset("pausedTimerId"));
					t &&
						(await new Promise((n) => {
							r.pausedTimerId = setTimeout(() => {
								(0, i.markIsPaused)(e).then(n, n);
		...
		28952: function (e, t, r) {
			...
			u.onInjected(function () {
				const e = async () => {
					l || ((l = !0), setTimeout(() => (l = !1), 1e3), s.internalEv.emit("conn.main_ready"));
				};
				"MAIN" === c.Stream.mode ? e() : (c.Cmd.on("main_stream_mode_ready", e), c.Cmd.on("main_stream_mode_ready_legacy", e));
			});
			let l = !1;
		},
		29044: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { subscribePresence: "subscribePresence" }, (e) => e.subscribePresence);
		},
		29116: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		29166: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setDescription = async function (e, t) {
					...
					if (!(null === (r = s.groupMetadata) || void 0 === r ? void 0 : r.canSetDescription()))
						throw new n.WPPError("you_are_not_allowed_set_group_description", `You are not allowed to set group description in ${s.id._serialized}`, { groupId: s.id.toString() });
					const u = await Promise.resolve((0, o.randomMessageId)());
					return await (0, o.sendSetGroupDescription)(s.id, t, u, null === (a = s.groupMetadata) || void 0 === a ? void 0 : a.descId), (s.groupMetadata.descId = u), (s.groupMetadata.desc = t), !0;
				});
			const n = r(62857),
				o = r(52757),
				i = r(2818);
		...
		31116: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setNotes = async function (e, t) {
					...
					if (!t) throw new i.WPPError("missing_content_for_notes", "Missing content for notes");
					if (r.isGroup) throw new i.WPPError("can_not_set_notes_for_groups", `You can not set notes for groups. ChatId: ${e}`);
					const s = await (0, a.retrieveOnlyNoteForChatJid)(r.id.toJid());
					return (
						await (0, a.addOrEditNoteAction)({ actionType: s ? "edit" : "add", chatJid: r.id.toJid(), content: t, createdAt: Math.floor(Date.now() / 1e3), id: null == s ? void 0 : s.id, noteType: "unstructured" }, !0),
		...
		33505: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { addOrEditNoteAction: "addOrEditNoteAction", retrieveOnlyNoteForChatJid: "retrieveOnlyNoteForChatJid" }, (e) => e.addOrEditNoteAction && e.retrieveOnlyNoteForChatJid);
		},
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.keepMessage = async function (e, t = !0) {
					...
					if (await (0, n.iAmAdmin)(r.id.remote)) {
						if (r.isExpired()) throw new o.WPPError("msg_expired", "This message has expired");
						return t
							? (await (0, a.keepMessage)(r, i.KIC_ENTRY_POINT_TYPE.CHAT), await (0, s.getMessageById)(e))
							: (await (0, a.undoKeepMessage)(r, { deleteExpired: !0 }, i.KIC_ENTRY_POINT_TYPE.CHAT), await (0, s.getMessageById)(e));
					}
		...
		34048: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { removeStatusMessage: ["removeStatusMessage"] }, (e) => e.removeStatusMessage);
		},
		34324: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { findChat: ["findChat", "findExistingChat"] }, (e) => e.findChat || e.findExistingChat);
		},
		34439: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		35090: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.editMessage = async function (e, t, r = {}) {
					...
					if (!c && !l) throw new n.WPPError("edit_message_error", "Cannot edit this message");
					let d = { type: "protocol", subtype: "message_edit", protocolMessageKey: u.id, body: t.trim(), caption: t.trim(), editMsgType: u.type };
					return (d = await (0, a.prepareRawMessage)(u.chat, d, r)), (d.latestEditMsgKey = d.id), (d.latestEditSenderTimestampMs = d.t), await (0, a.sendRawMessage)(null === (s = u.chat) || void 0 === s ? void 0 : s.id, d, r);
				});
			const n = r(62857),
				o = r(52757),
				i = r(74023),
		...
		35184: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendListMessage = async function (e, t) {
					const r = (t = Object.assign(Object.assign({}, d.defaultSendMessageOptions), t)).sections;
					...
					const n = { type: "list", list: { buttonText: t.buttonText, description: t.description || " ", title: t.title, footerText: t.footer, listType: 1, sections: r }, footer: t.footer };
		...
		35562: (e, t, r) => {
			...
			const n = r(1132),
				o = r(54993),
				i = r(65931);
			(0, o.exportModule)(t, "ServerPropsConstants", (e) => e.MAX_FILE_SIZE_BYTES),
				(0, n.injectFallbackModule)("ServerPropsConstants", {
					...
					MULTICAST_LIMIT_GLOBAL: (null === i.ServerProps || void 0 === i.ServerProps ? void 0 : i.ServerProps.frequentlyForwardedMax) || 5,
					PTT_PLAYBACK_SPEED_HIDE_DELAY: 1500,
					DEFAULT_MAX_FILE_SIZE_BYTES: 104857600,
					MAX_FILE_SIZE_BYTES: (null === i.ServerProps || void 0 === i.ServerProps ? void 0 : i.ServerProps.maxFileSize) || 104857600,
		...
		35824: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendEventMessage = async function (e, t) {
					var r;
					if ("string" == typeof (t = Object.assign(Object.assign({}, a.defaultSendMessageOptions), t)).callType && "voice" != t.callType && "video" != t.callType)
						throw new o.WPPError("callType_is_invalid", `Param callType: ${t.callType} is not a valid type call. Use voice or video`);
					...
					const c = Math.floor(u.getTime() / 1e3),
						l = {
							...
							author: null === (r = (0, n.getMyUserId)()) || void 0 === r ? void 0 : r.toString(),
							kind: "eventCreation",
							messageSecret: crypto.getRandomValues(new Uint8Array(32)),
							viewMode: "VISIBLE",
							eventJoinLink: "string" == typeof (null == t ? void 0 : t.callType) ? await (0, i.createEventCallLink)(t.startTime, t.callType) : void 0,
						};
		...
		36048: (e, t, r) => {
			...
			(0, o.exportModule)(
				t,
				{ getShouldAppearInList: "getShouldAppearInList", getPreviewMessage: "getPreviewMessage", getShowChangeNumberNotification: "getShowChangeNumberNotification", getShouldShowUnreadDivider: "getShouldShowUnreadDivider" },
				(e) => e.getShouldAppearInList && e.getPreviewMessage && e.getShowChangeNumberNotification && e.getShouldShowUnreadDivider
			),
				(0, o.exportModule)(t, { getHasUnread: "getHasUnread" }, (e) => e.getHasUnread),
				(0, n.injectFallbackModule)("getShouldAppearInList", {
					...
					getShowChangeNumberNotification: (e) => e.showChangeNumberNotification,
					getShouldShowUnreadDivider: (e) => e.shouldShowUnreadDivider,
				}),
				(0, n.injectFallbackModule)("getHasUnread", { getHasUnread: (e) => e.hasUnread });
		},
		36061: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
				...
				(t.sendVCardContactMessage = async function (e, t, r = {}) {
					...
					for (const e of t) {
						...
						i || (i = new o.ContactModel({ id: (0, n.assertWid)(t), name: r })),
							!r && i.id.equals(o.UserPrefs.getMaybeMeUser()) && (r = i.displayName),
							r && ((i = new o.ContactModel(i.attributes)), (i.name = r), Object.defineProperty(i, "formattedName", { value: r }), Object.defineProperty(i, "displayName", { value: r })),
							s.push(o.VCard.vcardFromContactModel(i));
					}
					const u = {};
					1 === s.length ? ((u.type = "vcard"), (u.body = s[0].vcard), (u.vcardFormattedName = s[0].displayName)) : ((u.type = "multi_vcard"), (u.vcardList = s));
		...
		36520: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setSubject = async function (e, t) {
					...
					if (!(null === (r = a.groupMetadata) || void 0 === r ? void 0 : r.canSetSubject()))
						throw new n.WPPError("you_are_not_allowed_set_group_subject", `You are not allowed to set group subject in ${a.id._serialized}`, { groupId: a.id.toString() });
					return await (0, o.sendSetGroupSubject)(a.id, t), (a.name = t), (a.formattedTitle = t), !0;
				});
			const n = r(62857),
				o = r(52757),
				i = r(2818);
		...
		36618: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.find = async function (e) {
					const t = (0, n.assertWid)(e),
						r = await (0, i.findChat)(t, t.isLid() ? "username_contactless_search" : "createChat");
			...
			(0, o.exportModule)(
				...
				(0, o.exportModule)(
					t,
					{
						...
						getDisplayNameOrPnForLid: ["getUserDisplayNameForLid", "getDisplayNameOrPnForLid"],
						getFormattedPhone: ["getFormattedPhone", "getFormattedUsernameOrPhone"],
						getFormattedUsernameOrPhone: ["getFormattedUsernameOrPhone", "getFormattedPhone"],
			...
			const n = /^[v^~<>=]*?(\d+)(?:\.([x*]|\d+)(?:\.([x*]|\d+)(?:\.([x*]|\d+))?(?:-([\da-z\-]+(?:\.[\da-z\-]+)*))?(?:\+[\da-z\-]+(?:\.[\da-z\-]+)*)?)?)?$/i,
				...
				c = (e, t) => {
					const r = o(e),
						...
						a = n.pop(),
						s = u(r, n);
					return 0 !== s ? s : i && a ? u(i.split("."), a.split(".")) : i || a ? (i ? -1 : 1) : 0;
				},
				l = (e, t, r) => {
					p(r);
					const n = c(e, t);
				...
				p = (e) => {
					...
					if (-1 === f.indexOf(e)) throw new Error(`Invalid operator, expected one of ${f.join("|")}`);
				},
				g = (e, t) => {
					if ((t = t.replace(/([><=]+)\s+/g, "$1")).includes("||")) return t.split("||").some((t) => g(e, t));
					if (t.includes(" - ")) {
						const [r, n] = t.split(" - ", 2);
						return g(e, `>=${r} <=${n}`);
					}
					...
					const h = b.findIndex((e) => "0" !== e) + 1,
						v = "~" === n ? 2 : h > 1 ? h : 1;
					return 0 === u(y.slice(0, v), b.slice(0, v)) && -1 !== u(y.slice(v), b.slice(v));
				},
				m = (e) => "string" == typeof e && /^[v\d]/.test(e) && n.test(e),
				y = (e) =>
					"string" == typeof e && /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$/.test(e);
		},
		38428: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		38636: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getQuotedMsg = async function (e) {
					const t = await (0, o.getMessageById)(e);
					if (!t.quotedStanzaID) throw new n.WPPError("message_not_have_a_reply", `Message ${e} does not have a reply`, { id: e });
		...
		39128: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getVotes = async function (e) {
					...
					if ("poll_creation" != r.type) throw new n.WPPError("msg_not_found", `Message ${t.toString()} not a poll`, { id: t.toString() });
		...
		39240: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { ProfilePicThumbCollection: ["ProfilePicThumbCollectionImpl", "ProfilePicThumbCollection"] }, (e) => e.ProfilePicThumbCollectionImpl || e.ProfilePicThumbCollection);
		},
		39453: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendLocationMessage = async function (e, t) {
					const r = (t = Object.assign(Object.assign({}, l.defaultSendMessageOptions), t)).name && t.address ? `${t.name}\n${t.address}` : t.name || t.address || "";
					...
					return (n = (0, f.prepareMessageButtons)(n, t)), await (0, d.sendRawMessage)(e, n, t);
				});
			const s = a(r(1132)),
				u = r(54993),
				c = r(52757),
		...
		39681: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.convertToFile = async function (e, t, r) {
					...
					if ("string" == typeof e) {
						let r = (0, i.default)(e);
						if ((!r && (0, a.isBase64)(e) && (r = (0, i.default)("data:;base64," + e)), !r)) throw "invalid_data_url";
						t || (t = r.contentType);
						const o = r.toBuffer();
				...
				(0, c.wrapModuleFunction)(l.updateDBForGroupAction, (t, ...r) => {
					...
					return (
						...
						queueMicrotask(() => {
							...
							"add" !== i || (!o.isInvite && "invite" !== o.reason) ? "remove" === i && t.some((e) => e.equals(n.author)) && (i = "leave") : (i = "join"),
		...
		40502: function (e, t, r) {
			...
			const s = a(r(1132)),
				u = r(54993),
				c = r(1703);
			(0, u.exportModule)(t, { sendRevokeGroupInviteCode: "sendRevokeGroupInviteCode" }, (e) => e.sendRevokeGroupInviteCode),
				s.injectFallbackModule("sendRevokeGroupInviteCode", { sendRevokeGroupInviteCode: async (e) => await (0, c.resetGroupInviteCode)(e).then((e) => e.code) });
		},
		40504: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		40649: function (e, t, r) {
			...
			for (const e of c) {
				...
				(0, u.exportModule)(t, { [e]: ["default", r] }, (e) => (e.default || e[r]) instanceof s[r]);
			}
			(0, u.exportModule)(t, { RecentStickerStore: ["default", "RecentStickerCollectionMd", "RecentStickerCollection"] }, (e) => e.RecentStickerCollection),
				(0, u.exportModule)(t, { StarredMsgStore: ["default", "AllStarredMsgsCollection"] }, (e) => e.StarredMsgCollection),
				(0, u.exportModule)(t, { StickerPackStore: ["default", "StickerPackCollectionMd", "StickerPackCollection"] }, (e) => e.StickerPackCollection),
				(0, u.exportModule)(t, { NewsletterStore: "default.NewsletterCollection" }, (e) => e.default.NewsletterCollection),
				(0, u.exportModule)(t, { StatusStore: "TextStatusCollection" }, (e) => e.TextStatusCollection),
				(0, u.exportModule)(t, { StatusV3Store: ["StatusV3Collection", "StatusCollection"] }, (e) => e.StatusV3CollectionImpl || e.StatusCollection),
				(0, u.exportModule)(t, { BlocklistStore: ["default", "BlocklistCollection"] }, (e) => e.BlocklistCollection),
				(0, u.exportModule)(t, { PresenceStore: ["PresenceCollectionImpl", "PresenceCollection"] }, (e) => e.PresenceCollectionImpl || e.PresenceCollection),
				(0, u.exportModule)(t, { CartStore: ["CartCollectionImpl", "CartCollection"] }, (e) => e.CartCollectionImpl || e.CartCollection),
				(0, u.exportModule)(t, { CatalogStore: ["CatalogCollectionImpl", "CatalogCollection"] }, (e) => e.CatalogCollectionImpl || e.CatalogCollection),
				(0, u.exportModule)(t, { EmojiVariantStore: ["EmojiVariantCollectionImpl", "EmojiVariantCollection"] }, (e) => e.EmojiVariantCollectionImpl || e.EmojiVariantCollection),
				(0, u.exportModule)(t, { LabelStore: ["LabelCollectionImpl", "LabelCollection"] }, (e) => e.LabelCollectionImpl || e.LabelCollection),
				(0, u.exportModule)(t, { MsgInfoStore: ["MsgInfoCollectionImpl", "MsgInfoCollection"] }, (e) => e.MsgInfoCollectionImpl || e.MsgInfoCollection),
				(0, u.exportModule)(t, { MuteStore: ["MuteCollectionImpl", "MuteCollection"] }, (e) => e.MuteCollectionImpl || e.MuteCollection),
				(0, u.exportModule)(t, { OrderStore: ["OrderCollectionImpl", "OrderCollection"] }, (e) => e.OrderCollectionImpl || e.OrderCollection),
				(0, u.exportModule)(t, { PinInChatStore: ["PinInChatCollectionImpl", "PinInChatCollection"] }, (e) => e.PinInChatCollectionImpl || e.PinInChatCollection),
				(0, u.exportModule)(t, { ProductMessageListStore: ["ProductMessageListCollectionImpl", "ProductMessageListCollection"] }, (e) => e.ProductMessageListCollectionImpl || e.ProductMessageListCollection),
				(0, u.exportModule)(t, { RecentEmojiStore: ["RecentEmojiCollectionImpl", "RecentEmojiCollection"] }, (e) => e.RecentEmojiCollectionImpl || e.RecentEmojiCollection),
				(0, u.exportModule)(t, { StickerSearchStore: ["StickerSearchCollectionImpl", "StickerSearchCollection"] }, (e) => e.StickerSearchCollectionImpl || e.StickerSearchCollection),
				(0, u.exportModule)(t, { BusinessProfileStore: ["BusinessProfileCollectionImpl", "BusinessProfileCollection"] }, (e) => e.BusinessProfileCollectionImpl || e.BusinessProfileCollection),
				(0, u.exportModule)(t, { BotProfileStore: ["BotProfileCollection"] }, (e) => e.BotProfileCollection),
				(0, u.exportModule)(t, { ProfilePicThumbStore: ["ProfilePicThumbCollection", "ProfilePicThumbCollectionImpl"] }, (e) => e.ProfilePicThumbCollection || e.ProfilePicThumbCollectionImpl),
				(0, u.exportModule)(t, { QuickReplyStore: ["QuickReplyCollectionImpl", "QuickReplyCollection"] }, (e) => e.QuickReplyCollectionImpl || e.QuickReplyCollection),
				(0, u.exportModule)(t, { NoteStore: ["NoteCollection"] }, (e) => e.NoteCollection),
				(0, u.exportModule)(t, { ReactionsStore: ["ReactionsCollectionImpl", "ReactionsCollection"] }, (e) => e.ReactionsCollectionImpl || e.ReactionsCollection),
				(0, u.exportModule)(t, { ChatStore: ["ChatCollection"] }, (e) => e.ChatCollection);
		},
		40688: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		40866: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.editLabel = async function (e, t = {}) {
					...
					if (!r) throw new o.WPPError("label_not_exist", `Label with id ${e} not exist`);
					let u = t.labelColor || void 0;
					if (u && ("string" == typeof u && u.length > 2 && (u = (0, a.getAllLabelColors)().findIndex((e) => e === u)), (u = parseInt(u.toString())), !(await (0, s.colorIsInLabelPalette)(u))))
						throw new o.WPPError("color_not_in_pallet", "Color not in pallet");
					return await (0, a.labelEditAction)(e, t.name || r.name, 0, u || r.colorIndex), await (0, s.getLabelById)(e.toString());
				});
			const n = r(41687),
				o = r(62857),
				i = r(14647),
		...
		40923: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.editBusinessProfile = async function (e) {
					...
					return await n.BusinessProfileStore.fetchBizProfile(n.UserPrefs.getMaybeMeUser());
				});
			const n = r(14647),
				o = r(52757);
		},
		...
		40954: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendGroupInviteMessage = async function (e, t) {
					if (!(t = Object.assign(Object.assign({}, a.defaultSendMessageOptions), t)).groupName) {
					...
					u = t.inviteCodeExpiration
						? { type: "groups_v4_invite", inviteGrpJpegThum: t.jpegThumbnail, inviteCode: t.inviteCode, inviteCodeExp: t.inviteCodeExpiration || "", inviteGrp: t.groupId, inviteGrpName: t.groupName, comment: t.caption }
						: {
							type: "chat",
							subtype: "url",
		...
		41898: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setOnline = async function (e) {
					if ("string" != typeof e || !Object.values(i).includes(e)) throw new n.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set online privacy`, { value: e });
					return await (0, o.setPrivacyForOneCategory)({ name: "online", value: e }), (0, o.getUserPrivacySettings)().online;
				});
			const n = r(62857),
				o = r(52757);
			var i;
		...
		41968: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				...
				(e) =>
					(e.getWhatsAppWebExternalBetaJoinedIdb && e.setWhatsAppWebExternalBetaDirtyBitIdb && e.setWhatsAppWebExternalBetaJoinedIdb) ||
					(e.getWhatsAppWebBetaJoined && e.setWhatsAppWebBetaDirtyBit && e.setWhatsAppWebBetaJoined)
			);
		},
		42217: (e, t, r) => {
			"use strict";
		...
		42556: function (e, t, r) {
			...
			const s = r(38385),
				...
				c = r(54993),
				l = r(97718);
			(0, c.exportModule)(t, { sendCreateGroup: "sendCreateGroup" }, (e) => e.sendCreateGroup),
				u.injectFallbackModule("sendCreateGroup", {
					sendCreateGroup: async (e, t, r, n) =>
						(0, s.compare)(self.Debug.VERSION, "2.3000.1014489107", ">=")
							? await (0, l.createGroup)({ title: e, ephemeralDuration: r || 0, restrict: !0, announce: !0, membershipApprovalMode: !1, memberAddMode: !0, parentGroupId: n }, t).then((e) => ({
								gid: e.wid,
								participants: e.participants.map((e) => ({ userWid: e.wid, code: null != e.error ? e.error.toString() : "200", invite_code: e.invite_code, invite_code_exp: e.invite_code_exp })),
							}))
		...
		42992: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendPixKeyMessage = async function (e, t, r) {
					if (!(e && t.keyType && t.name && t.key)) throw new n.WPPError("parameter_not_fount", "Please, send all the required parameters");
					r = Object.assign(Object.assign({}, o.defaultSendMessageOptions), r);
					const a = {
						order: {
							items: [{ name: "", retailer_id: `custom-item-${(0, n.generateOrderUniqueId)()}`, amount: { offset: 1, value: 0 }, quantity: 0 }],
						...
						payment_settings: [
							{ type: "pix_static_code", pix_static_code: { key_type: t.keyType, merchant_name: t.name, key: t.key } },
							{ type: "cards", cards: { enabled: !1 } },
						],
						external_payment_configurations: [{ payment_instruction: t.instructions || "", type: "payment_instruction" }],
						additional_note: "",
						currency: "BRL",
						type: "physical-goods",
					},
						s = {
							...
							interactivePayload: { buttons: [{ buttonParamsJson: JSON.stringify(a), name: "payment_info" }], messageVersion: 1 },
		...
		43394: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.prepareMessageButtons = function (e, t) {
					...
					if (!Array.isArray(t.buttons)) throw new s.WPPError("buttons_not_a_array", "Buttons options is not a array");
					if ("chat" !== e.type && t.buttons.length > 2) throw new s.WPPError("not_alowed_more_then_three_buttons", "Not allowed more then three buttons in file messages");
					if (0 === t.buttons.length || t.buttons.length > 3) throw new s.WPPError("buttons_must_between_1_and_3_options", "Buttons options must have between 1 and 3 options");
					if (t.buttons.find((e) => e.phoneNumber || e.url) && t.buttons.find((e) => e.id && e.text))
						throw new s.WPPError("reply_and_cta_btn_not_allowed", "It is not possible to send reply buttons and action buttons togetherButtons options must have between 1 and 3 options");
					return (
						(e.title = t.title),
						(e.footer = t.footer),
						(e.interactiveMessage = {
							...
							nativeFlowMessage: {
								buttons: t.buttons.map((e, t) =>
									"phoneNumber" in e
										? { name: "cta_call", buttonParamsJson: JSON.stringify({ display_text: e.text, phone_number: e.phoneNumber }) }
										: "url" in e
											? { name: "cta_url", buttonParamsJson: JSON.stringify({ display_text: e.text, url: e.url, merchant_url: e.url }) }
											: "code" in e
												? { name: "cta_copy", buttonParamsJson: JSON.stringify({ display_text: e.text, copy_code: e.code }) }
												: "raw" in e
													? e.raw
													: { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: e.text, id: e.id || `${t}` }) }
								),
							},
						}),
						(e.isFromTemplate = !0),
						...
						(e.hydratedButtons = t.buttons.map((e, t) =>
							"phoneNumber" in e
								...
								: "url" in e
									...
									: "code" in e
										? { index: t, urlButton: { displayText: e.text, url: `https://www.whatsapp.com/otp/code/?otp_type=COPY_CODE&code=otp${e.code}` } }
						...
						e.buttons.add(
							e.hydratedButtons.map((e, t) => {
								...
								return e.urlButton
									...
									: e.callButton
										? new c.TemplateButtonModel({ id: a, displayText: e.callButton.displayText, phoneNumber: e.callButton.phoneNumber, subtype: "call" })
										: new c.TemplateButtonModel({
											id: a,
			...
			u.onFullReady(() => {
				(0, d.wrapModuleFunction)(f.createMsgProtobuf, (e, ...t) => {
					...
					const [a] = t,
						s = e(...t);
					if (void 0 !== (null === (n = null === (r = a.interactiveMessage) || void 0 === r ? void 0 : r.nativeFlowMessage) || void 0 === n ? void 0 : n.buttons)) {
						...
						for (let t of e)
							if (t in s) {
								...
								"documentWithCaptionMessage" === t && (t = "documentMessage"),
									(a.interactiveMessage.header = Object.assign(Object.assign({}, a.interactiveMessage.header), {
										[`${t}`]: (null === (i = null === (o = s[e]) || void 0 === o ? void 0 : o.message) || void 0 === i ? void 0 : i.documentMessage) || s[e],
										hasMediaAttachment: !0,
									})),
									delete s[e];
								break;
							}
						void 0 !== s.extendedTextMessage && delete s.extendedTextMessage, void 0 !== s.conversation && delete s.conversation, (s.viewOnceMessage = { message: { interactiveMessage: a.interactiveMessage } });
					}
					return s;
				}),
					...
					(0, d.wrapModuleFunction)(f.mediaTypeFromProtobuf, (e, ...t) => {
						var r, n, o, i, a, s;
						const [u] = t;
						return (null === (o = null === (n = null === (r = u.documentWithCaptionMessage) || void 0 === r ? void 0 : r.message) || void 0 === n ? void 0 : n.templateMessage) || void 0 === o ? void 0 : o.hydratedTemplate)
							? e(null === (s = null === (a = null === (i = u.documentWithCaptionMessage) || void 0 === i ? void 0 : i.message) || void 0 === a ? void 0 : a.templateMessage) || void 0 === s ? void 0 : s.hydratedTemplate)
							: e(...t);
					}),
					(0, d.wrapModuleFunction)(f.typeAttributeFromProtobuf, (e, ...t) => {
						var r, n, o, i, a, s, u, c, l, d;
						const [f] = t;
						if (null === (r = null == f ? void 0 : f.viewOnceMessage) || void 0 === r ? void 0 : r.interactiveMessage) {
							const e = Object.keys(null === (n = null == f ? void 0 : f.viewOnceMessage) || void 0 === n ? void 0 : n.interactiveMessage);
						...
						if (
							null === (a = null === (i = null === (o = null == f ? void 0 : f.documentWithCaptionMessage) || void 0 === o ? void 0 : o.message) || void 0 === i ? void 0 : i.templateMessage) || void 0 === a
								? void 0
								: a.hydratedTemplate
						) {
							const e = Object.keys(
								null === (c = null === (u = null === (s = null == f ? void 0 : f.documentWithCaptionMessage) || void 0 === s ? void 0 : s.message) || void 0 === u ? void 0 : u.templateMessage) || void 0 === c
									? void 0
									: c.hydratedTemplate
							);
							return ["documentMessage", "imageMessage", "locationMessage", "videoMessage"].some((t) => e.includes(t)) ? "media" : "text";
						}
						return 1 === (null === (l = null == f ? void 0 : f.buttonsMessage) || void 0 === l ? void 0 : l.headerType) || 2 === (null === (d = null == f ? void 0 : f.buttonsMessage) || void 0 === d ? void 0 : d.headerType)
							? "text"
							: e(...t);
					}),
					(0, d.wrapModuleFunction)(f.createFanoutMsgStanza, async (e, ...t) => {
						var r, n;
						let o = null;
						...
						let a = await e(...t);
						if (
							((null === (n = null === (r = null == i ? void 0 : i.viewOnceMessage) || void 0 === r ? void 0 : r.message) || void 0 === n ? void 0 : n.interactiveMessage) &&
						...
						let l = !1;
						return Array.isArray(u.content) ? (l = !!u.content.find((e) => e.tag === (null == o ? void 0 : o.tag))) : (u.content = []), l || u.content.push(o), a;
					}),
		...
		43410: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				t,
				{ sendSetGroupSubject: ["sendSetGroupSubject", "setGroupSubject"], sendSetGroupDescription: ["sendSetGroupDescription", "setGroupDescription"], sendSetGroupProperty: ["sendSetGroupProperty", "setGroupProperty"] },
		...
		43669: function (e, t, r) {
			...
			const s = a(r(1132)),
				u = r(54993),
				c = r(52757);
			(0, u.exportModule)(t, { HistorySyncProgressModel: "HistorySyncProgressModel" }, (e) => e.HistorySyncProgressModel);
			const l = {};
			Object.defineProperty(l, "HistorySyncProgressModel", { configurable: !0, enumerable: !0, get: () => (0, c.getHistorySyncProgress)().constructor }), s.injectFallbackModule("HistorySyncProgressModel", l);
		},
		43859: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setChatList = async function (e, t) {
					if (((p = e), !e)) throw new s.WPPError("send_type_filter", "Please send a valid type filter");
					if (e == g.LABELS && !t) throw new s.WPPError("send_labelId", "Please send a valid label id");
					if (e == g.CUSTOM && !t) throw new s.WPPError("send_ids", "Please send a valid ids");
					"string" == typeof t && (t = [t]);
					return e == g.CUSTOM && t
						? ((f = t), c.Cmd.trigger("set_active_filter", "unread"), c.Cmd.trigger("set_active_filter"), { type: e, list: f })
						: e == g.ALL
							? (c.Cmd.trigger("set_active_filter"), { type: e })
							: e == g.LABELS
								? (c.Cmd.trigger("set_active_filter", g.LABELS, t[0]), { type: e })
								: (c.Cmd.trigger("set_active_filter", "unread"), c.Cmd.trigger("set_active_filter", e), { type: e });
				}),
				(t.wrapShouldAppearFunction = function (e, t) {
					const r = (...r) => t(e, ...r);
					return Object.defineProperties(r, Object.getOwnPropertyDescriptors(d.getShouldAppearInList)), r;
				});
			const s = r(62857),
				u = a(r(1132)),
				c = r(14647),
			...
			!(function (e) {
				...
				u.onFullReady(function () {
					(0, l.wrapModuleFunction)(d.getShouldAppearInList, (e, ...t) => {
						var r;
						const [n] = t;
						return p === g.CUSTOM ? !!f.includes(null === (r = n.id) || void 0 === r ? void 0 : r.toString()) : e(...t);
					}),
						(0, l.wrapModuleFunction)(d.isFilterExcludedFromSearchTreatmentInInboxFlow, async (e, ...t) => {
							const [r] = t;
							return r === g.LABELS || (p == g.CUSTOM && ((p = g.ALL), c.Cmd.trigger("set_active_filter", "default"), c.Cmd.trigger("set_active_filter", r)), (p = g.ALL), c.Cmd.trigger("set_active_filter", r)), e(...t);
						});
				}, 1e3);
		},
		43871: (e, t, r) => {
		...
		44190: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendRawMessage = async function (e, t, r = {}) {
					var n;
					const o = (r = Object.assign(Object.assign({}, d.defaultSendMessageOptions), r)).createChat ? await (0, i.assertFindChat)(e) : (0, i.assertGetChat)(e);
					if (o.isGroup && o.isParentGroup) {
						const e = u.GroupMetadataStore.get(null === (n = o.id) || void 0 === n ? void 0 : n.toString());
					...
					(t = await (0, f.prepareRawMessage)(o, t, r)), r.markIsRead && (p("marking chat is read before send message"), await (0, f.markIsRead)(o.id).catch(() => null));
					...
					if ((null == o ? void 0 : o.isNewsletter) && t.type) {
						if (!["chat", "image", "video", "poll_creation"].includes(t.type))
							throw new s.WPPError("type_not_valid_for_newsletter", 'Please, send a valid type for send message to newsletter. Valid types: "chat", "image", "video", "poll_creation"');
						const e = new u.MsgModel(t);
						await (0, l.addNewsletterMsgsRecords)([await (0, l.msgDataFromMsgModel)(e)]);
						const r = await (0, l.sendNewsletterMessageJob)({
							type: "chat" != t.type || t.editMsgType ? ("poll_creation" == t.type ? "pollCreation" : "media") : "text",
							msgData: t,
							msg: new u.MsgModel(t),
							newsletterJid: o.id.toString(),
						});
						o.msgs.add(e), r.success && ((e.t = r.ack.t), (e.serverId = r.serverId)), e.updateAck(c.ACK.SENT, !0), await (0, l.updateNewsletterMsgRecord)(e), (g = [e, "OK"]);
					} else if ("protocol" === t.type && "message_edit" === t.subtype) {
						const e = await (0, f.getMessageById)(t.protocolMessageKey);
						await (0, l.addAndSendMessageEdit)(e, t), (g = [await (0, f.getMessageById)(t.protocolMessageKey), null]);
					...
					return Object.assign(
						Object.assign(
							Object.assign(Object.assign({ id: m.id.toString(), ack: m.ack }, m.latestEditMsgKey && { latestEditMsgKey: m.latestEditMsgKey }), m.from && { from: m.from.toString() }),
		...
		44266: (e, t, r) => {
			...
			(t.UINT8 = { len: 1, get: (e, t) => i(e).getUint8(t), put: (e, t, r) => (i(e).setUint8(t, r), t + 1) }),
				(t.UINT16_LE = { len: 2, get: (e, t) => i(e).getUint16(t, !0), put: (e, t, r) => (i(e).setUint16(t, r, !0), t + 2) }),
				(t.UINT16_BE = { len: 2, get: (e, t) => i(e).getUint16(t), put: (e, t, r) => (i(e).setUint16(t, r), t + 2) }),
				(t.UINT24_LE = {
					len: 3,
					get(e, t) {
						const r = i(e);
				...
				(t.UINT32_LE = { len: 4, get: (e, t) => i(e).getUint32(t, !0), put: (e, t, r) => (i(e).setUint32(t, r, !0), t + 4) }),
				(t.UINT32_BE = { len: 4, get: (e, t) => i(e).getUint32(t), put: (e, t, r) => (i(e).setUint32(t, r), t + 4) }),
				(t.INT8 = { len: 1, get: (e, t) => i(e).getInt8(t), put: (e, t, r) => (i(e).setInt8(t, r), t + 1) }),
				(t.INT16_BE = { len: 2, get: (e, t) => i(e).getInt16(t), put: (e, t, r) => (i(e).setInt16(t, r), t + 2) }),
				(t.INT16_LE = { len: 2, get: (e, t) => i(e).getInt16(t, !0), put: (e, t, r) => (i(e).setInt16(t, r, !0), t + 2) }),
				(t.INT24_LE = {
					len: 3,
					get(e, r) {
				...
				(t.INT32_BE = { len: 4, get: (e, t) => i(e).getInt32(t), put: (e, t, r) => (i(e).setInt32(t, r), t + 4) }),
				(t.INT32_LE = { len: 4, get: (e, t) => i(e).getInt32(t, !0), put: (e, t, r) => (i(e).setInt32(t, r, !0), t + 4) }),
				(t.UINT64_LE = { len: 8, get: (e, t) => i(e).getBigUint64(t, !0), put: (e, t, r) => (i(e).setBigUint64(t, r, !0), t + 8) }),
				(t.INT64_LE = { len: 8, get: (e, t) => i(e).getBigInt64(t, !0), put: (e, t, r) => (i(e).setBigInt64(t, r, !0), t + 8) }),
				(t.UINT64_BE = { len: 8, get: (e, t) => i(e).getBigUint64(t), put: (e, t, r) => (i(e).setBigUint64(t, r), t + 8) }),
				(t.INT64_BE = { len: 8, get: (e, t) => i(e).getBigInt64(t), put: (e, t, r) => (i(e).setBigInt64(t, r), t + 8) }),
				(t.Float16_BE = {
					len: 2,
					get(e, t) {
				...
				(t.Float32_BE = { len: 4, get: (e, t) => i(e).getFloat32(t), put: (e, t, r) => (i(e).setFloat32(t, r), t + 4) }),
				(t.Float32_LE = { len: 4, get: (e, t) => i(e).getFloat32(t, !0), put: (e, t, r) => (i(e).setFloat32(t, r, !0), t + 4) }),
				(t.Float64_BE = { len: 8, get: (e, t) => i(e).getFloat64(t), put: (e, t, r) => (i(e).setFloat64(t, r), t + 8) }),
				(t.Float64_LE = { len: 8, get: (e, t) => i(e).getFloat64(t, !0), put: (e, t, r) => (i(e).setFloat64(t, r, !0), t + 8) }),
				(t.Float80_BE = {
					len: 10,
					get(e, t) {
			...
			class a {
				...
				static decode(e, t, r) {
					let n = "";
					for (let o = t; o < r; ++o) n += a.codePointToString(a.singleByteDecoder(e[o]));
					return n;
				}
		...
		45195: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getGroupInfoFromInviteCode = async function (e) {
					...
					if (!a) throw new n.WPPError("invalid_invite_code", "Invalid Invite Code", { inviteCode: e });
					return Object.assign(Object.assign({}, a), {
						descOwner: null === (t = a.descOwner) || void 0 === t ? void 0 : t.toString(),
						id: a.id.toString(),
						owner: null === (r = a.owner) || void 0 === r ? void 0 : r.toString(),
						participants: a.participants.map((e) => Object.assign(Object.assign({}, e), { id: e.id.toString() })),
						subjectOwner: null === (i = a.subjectOwner) || void 0 === i ? void 0 : i.toString(),
					});
				});
			const n = r(62857),
				o = r(52757);
		...
		45218: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.updateParticipants = async function (e) {
					...
					const a = r.map((e) => new i.ParticipantModel({ id: e, isAdmin: !1, isSuperAdmin: !1 })),
						s = i.WidFactory.createWid("status@broadcast");
					await i.functions.updateParticipants({ group: s, participants: a, version: Date.now(), isOffline: !1 }), localStorage.setItem("wpp-status-participants", t);
				});
			const n = r(41687),
				o = r(91491),
				i = r(14647);
		...
		45988: (e, t, r) => {
			...
			function o(e) {
				...
				return { ip: [r.getUint8(0), r.getUint8(1), r.getUint8(2), r.getUint8(3)], port: r.getUint16(4) };
			}
		},
		46820: (e, t, r) => {
			"use strict";
		...
		46915: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { createChat: "createChat" }, (e) => e.createChat);
		},
		47440: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		47868: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const s = a(r(1132));
			(0, r(54993).exportModule)(t, "IsOfficialClient", (e) => void 0 !== e.isOfficialClient), s.injectFallbackModule("IsOfficialClient", { isOfficialClient: !0 });
		},
		48077: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { CALL_STATES: ["CALL_STATES", "CallState"] }, (e) => e.CALL_STATES || e.CallState);
		},
		48157: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.downloadMedia = async function e(t) {
					...
					if (!a.mediaData) throw new n.WPPError("message_not_contains_media", `Message ${t} not contains media`, { id: t });
					await a.downloadMedia({ downloadEvenIfExpensive: !0, rmrReason: 1, isUserInitiated: !0 });
					let s = null;
					a.mediaData.filehash && (s = o.MediaBlobCache.get(a.mediaData.filehash));
					!s && a.mediaData.mediaBlob && (s = a.mediaData.mediaBlob.forceToBlob());
					if (!s && "VIDEO" === (null === (r = a.mediaObject) || void 0 === r ? void 0 : r.type))
						try {
							return (a.type = "document"), (a.mediaObject.type = "DOCUMENT"), await e(t);
						} finally {
							(a.type = "video"), (a.mediaObject.type = "VIDEO");
						}
					if (!s) throw { error: !0, code: "media_not_found", message: "Media not found" };
					return s;
				});
			const n = r(62857),
				o = r(14647),
		...
		48287: (e, t, r) => {
			...
			function c(e, t, r) {
				...
				const o = (function (e) {
					...
					if (void 0 !== e.length) return "number" != typeof e.length || Q(e.length) ? s(0) : f(e);
						...
						switch (a) {
							...
							case 4:
								(r = e[o + 1]),
									(n = e[o + 2]),
									(s = e[o + 3]),
									128 == (192 & r) && 128 == (192 & n) && 128 == (192 & s) && ((u = ((15 & t) << 18) | ((63 & r) << 12) | ((63 & n) << 6) | (63 & s)), u > 65535 && u < 1114112 && (i = u));
						}
					}
			...
			(u.TYPED_ARRAY_SUPPORT = (function () {
				...
				(u.compare = function (e, t) {
					if ((H(e, Uint8Array) && (e = u.from(e, e.offset, e.byteLength)), H(t, Uint8Array) && (t = u.from(t, t.offset, t.byteLength)), !u.isBuffer(e) || !u.isBuffer(t)))
				...
				(u.concat = function (e, t) {
					...
					for (r = 0; r < e.length; ++r) {
						let t = e[r];
						if (H(t, Uint8Array)) o + t.length > n.length ? (u.isBuffer(t) || (t = u.from(t)), t.copy(n, o)) : Uint8Array.prototype.set.call(n, t, o);
						else {
				...
				(u.prototype.compare = function (e, t, r, n, o) {
					if ((H(e, Uint8Array) && (e = u.from(e, e.offset, e.byteLength)), !u.isBuffer(e))) throw new TypeError('The "target" argument must be one of type Buffer or Uint8Array. Received type ' + typeof e);
			...
			function G(e, t, r, n, o, i) {
				if (e > r || e < t) {
					...
					let o;
					throw (
						((o = i > 3 ? (0 === t || t === BigInt(0) ? `>= 0${n} and < 2${n} ** ${8 * (i + 1)}${n}` : `>= -(2${n} ** ${8 * (i + 1) - 1}${n}) and < 2 ** ${8 * (i + 1) - 1}${n}`) : `>= ${t}${n} and <= ${r}${n}`),
							new U.ERR_OUT_OF_RANGE("value", o, e))
					);
				}
				!(function (e, t, r) {
					q(t, "offset"), (void 0 !== e[t] && void 0 !== e[t + r]) || z(t, e.length - (r + 1));
				})(n, o, i);
			}
			function q(e, t) {
			...
			function z(e, t, r) {
				if (Math.floor(e) !== e) throw (q(e, r), new U.ERR_OUT_OF_RANGE(r || "offset", "an integer", e));
				...
				D(
					...
					function (e, t, r) {
						let n = `The value of "${e}" is out of range.`,
							o = r;
						return (
							Number.isInteger(r) && Math.abs(r) > 2 ** 32 ? (o = W(String(r))) : "bigint" == typeof r && ((o = String(r)), (r > BigInt(2) ** BigInt(32) || r < -(BigInt(2) ** BigInt(32))) && (o = W(o)), (o += "n")),
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getMessages = async function (e, t = {}) {
					...
					const c = "after" === t.direction ? "after" : "before",
						l = t.id || (null === (r = s.lastReceivedKey) || void 0 === r ? void 0 : r.toString());
					if (t.onlyUnread) {
		...
		50175: (e, t) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.defaultSendMessageOptions = { createChat: !1, detectMentioned: !0, linkPreview: !0, markIsRead: !0, waitForAck: !0, delay: 0 });
		},
		50271: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
			...
			u.onInjected(() => {
				c.MsgStore.on("add", (e) => {
					e.isNewMsg &&
						e.isLive &&
						queueMicrotask(() => {
							s.internalEv.emit("chat.live_location_start", { id: e.sender, msgId: e.id, chat: e.chat.id, lat: e.lat, lng: e.lng, accuracy: e.accuracy, speed: e.speed, degrees: e.degrees, shareDuration: e.shareDuration });
						});
				});
			});
		},
				...
				(t.setLimit = function (e, t) {
					switch (e) {
						...
						case "maxFileSize":
							...
							return (c.ServerPropsConstants.MAX_FILE_SIZE_BYTES = t), c.ServerPropsConstants.MAX_FILE_SIZE_BYTES;
						case "maxShare":
							if ("number" != typeof t) throw new s.WPPError("maxShare_error", "Value type invalid!");
							return (c.ServerPropsConstants.MULTICAST_LIMIT_GLOBAL = t), c.ServerPropsConstants.MULTICAST_LIMIT_GLOBAL;
		...
		53011: (e, t, r) => {
			...
			!(function (o) {
				...
				function y(e, t) {
					for (var r = {}, n = e.length, i = t ? t.length : 0, a = 0; a < n; a++) r[e[a]] = a < i ? t[a] : o;
					return r;
				}
				function b(e, t, r) {
					var n, o;
					if (
						((this._emitter = e),
							...
							t.addEventListener ? ((n = t.addEventListener), (o = t.removeEventListener)) : t.addListener ? ((n = t.addListener), (o = t.removeListener)) : t.on && ((n = t.on), (o = t.off)),
							!n && !o)
					)
				...
				function h(e, t, r, n) {
					...
					for (var p = 0; p < d; p++) {
						if (((s = l[p]), !n && !i.call(t, s))) throw Error('Unknown "' + s + '" option');
				...
				function _(e) {
					...
					return 1 === r
						...
						: 2 === r
							...
							: function (n, o) {
								for (var i = typeof n, a = r; a-- > 0;) if (i === e[a]) return n;
								o(t);
							};
				}
				Object.assign(b.prototype, {
					subscribe: function (e, t, r) {
						var n = this,
							...
							s = function () {
								...
								r ? !1 !== r.call(o, a) && i.emit.apply(i, [a.name].concat(n)) : i.emit.apply(i, [t].concat(n));
							};
						...
						this._listenersCount++,
							i._newListener && i._removeListener && !n._onNewListener
								? ((this._onNewListener = function (r) {
									r === t && null === a[e] && ((a[e] = s), n._on.call(o, e, s));
								}),
									i.on("newListener", this._onNewListener),
									(this._onRemoveListener = function (r) {
										r === t && !i.hasListeners(r) && a[e] && ((a[e] = null), n._off.call(o, e, s));
									}),
									(a[e] = null),
					...
					unsubscribe: function (e) {
						...
						function c() {
							o._onNewListener && (a.off("newListener", o._onNewListener), a.off("removeListener", o._onRemoveListener), (o._onNewListener = null), (o._onRemoveListener = null));
							var e = O.call(a, o);
						...
						if (e) {
							...
							for (n = (r = d(i)).length; n-- > 0;) (e = r[n]), s.call(u, e, i[e]);
				...
				function j(e, t, r, n, o) {
					...
					if (n === o) r._listeners && ("function" == typeof r._listeners ? (e && e.push(r._listeners), (_ = [r])) : (e && e.push.apply(e, r._listeners), (_ = [r])));
						...
						if ("*" === P) {
							for (s = (h = d(r)).length; s-- > 0;) "_listeners" !== (p = h[s]) && (v = j(e, t, r[p], n + 1, o)) && (_ ? _.push.apply(_, v) : (_ = v));
							return _;
						}
						if ("**" === P) {
							for ((b = n + 1 === o || (n + 2 === o && "*" === M)) && r._listeners && (_ = j(e, t, r, o, o)), s = (h = d(r)).length; s-- > 0;)
								"_listeners" !== (p = h[s]) &&
									("*" === p || "**" === p ? (r[p]._listeners && !b && (v = j(e, t, r[p], o, o)) && (_ ? _.push.apply(_, v) : (_ = v)), (v = j(e, t, r[p], n, o))) : (v = j(e, t, r[p], p === M ? n + 2 : n, o)),
					...
					if (((g = r["*"]) && j(e, t, g, n + 1, o), (m = r["**"])))
						if (n < o)
							for (m._listeners && j(e, t, m, o, o), s = (h = d(m)).length; s-- > 0;)
								"_listeners" !== (p = h[s]) && (p === M ? j(e, t, m[p], n + 2, o) : p === P ? j(e, t, m[p], n + 1, o) : (((y = {})[p] = m[p]), j(e, t, { "**": y }, n + 1, o)));
						else m._listeners ? j(e, t, m, o, o) : m["*"] && m["*"]._listeners && j(e, t, m["*"], o, o);
					return _;
				}
				function S(e, t, r) {
					var n,
					...
					if (i > 1) for (n = 0; n + 1 < i; n++) if ("**" === o[n] && "**" === o[n + 1]) return;
					var c,
						l = this.listenerTree;
					for (n = 0; n < i; n++)
							...
							return (
								l._listeners
									? ("function" == typeof l._listeners && (l._listeners = [l._listeners]),
										...
										!l._listeners.warned && this._maxListeners > 0 && l._listeners.length > this._maxListeners && ((l._listeners.warned = !0), g.call(this, l._listeners.length, c)))
									: (l._listeners = t),
								!0
							);
					return !0;
				...
				function C(e, t, r, n) {
					for (var o, i, a, s, u = d(e), c = u.length, l = e._listeners; c-- > 0;)
						(o = e[(i = u[c])]), (a = "_listeners" === i ? r : r ? r.concat(i) : [i]), (s = n || "symbol" == typeof i), l && t.push(s ? a : a.join(this.delimiter)), "object" == typeof o && C.call(this, o, t, a, s);
					return t;
				}
				function x(e) {
					for (var t, r, n, o = d(e), i = o.length; i-- > 0;) (t = e[(r = o[i])]) && ((n = !0), "_listeners" === r || x(t) || delete e[r]);
					return n;
				}
				function I(e, t, r) {
				...
				(I.prototype.off = function () {
					...
					(A.prototype.listenTo = function (e, t, r) {
						...
						function i(t) {
							...
							for (var s, u = d(t), c = u.length, l = "function" == typeof i, f = 0; f < c; f++) (s = u[f]), o.subscribe(s, t[s] || s, l ? i : i && i[s]);
						}
						return (r = h(r, { on: o, off: o, reducers: o }, { on: P, off: P, reducers: M })), a(t) ? i(y(t)) : i("string" == typeof t ? y(t.split(/\s+/)) : t), this;
					}),
					...
					(A.prototype.emit = function () {
						...
						if (s && ((e = a), "newListener" !== a && "removeListener" !== a && "object" == typeof a)) {
							if (((r = a.length), u))
								for (n = 0; n < r; n++)
						...
						if (c && c.length) {
							if (l > 3) for (t = new Array(l - 1), o = 1; o < l; o++) t[o - 1] = arguments[o];
						...
						if (!this.ignoreErrors && !this._all && "error" === a) throw arguments[1] instanceof Error ? arguments[1] : new Error("Uncaught, unspecified 'error' event.");
						return !!this._all;
					}),
					(A.prototype.emitAsync = function () {
						...
						if ((s ? ((c = []), j.call(this, c, e, this.listenerTree, 0)) : (c = this._events[a]), "function" == typeof c))
							...
							if (((c = c.slice()), d > 3)) for (r = new Array(d - 1), i = 1; i < d; i++) r[i - 1] = arguments[i];
							...
							this.wildcard
								...
								: (this._events[e]
									? ("function" == typeof this._events[e] && (this._events[e] = [this._events[e]]),
										...
										!this._events[e].warned && this._maxListeners > 0 && this._events[e].length > this._maxListeners && ((this._events[e].warned = !0), g.call(this, this._events[e].length, e)))
									: (this._events[e] = t),
									a)
						);
					}),
					(A.prototype.off = function (e, t) {
						...
						for (var i = 0; i < n.length; i++) {
							...
							if (((r = s._listeners), a(r))) {
								...
								if (u < 0) continue;
								return (
									this.wildcard ? s._listeners.splice(u, 1) : this._events[e].splice(u, 1),
					...
					(A.prototype.offAny = function (e) {
						...
						if (e && this._all && this._all.length > 0) {
							for (r = 0, n = (t = this._all).length; r < n; r++) if (e === t[r]) return t.splice(r, 1), this._removeListener && this.emit("removeListenerAny", e), this;
						} else {
							if (((t = this._all), this._removeListener)) for (r = 0, n = t.length; r < n; r++) this.emit("removeListenerAny", t[r]);
							this._all = [];
						}
						return this;
					}),
					...
					(A.prototype.listeners = function (e) {
						...
						if (e === o) {
							...
							if (!s) return [];
							for (i = (t = d(s)).length, n = []; i-- > 0;) "function" == typeof (r = s[t[i]]) ? n.push(r) : n.push.apply(n, r);
							return n;
						}
						if (this.wildcard) {
					...
					(A.prototype.waitFor = function (e, t) {
						...
						return (
							...
							w(
								(t = h(t, { timeout: 0, filter: o, handleError: !1, Promise, overload: !1 }, { filter: P, Promise: v })).Promise,
								function (n, o, i) {
									function a() {
										var i = t.filter;
				...
				Object.defineProperties(A, {
					...
					once: {
						value: function (e, t, r) {
							return w(
								(r = h(r, { Promise, timeout: 0, overload: !1 }, { Promise: v })).Promise,
								function (r, n, o) {
									var i;
									...
									var a,
										s = function () {
											a && e.removeListener("error", a), r(m.apply(null, arguments));
										};
									"error" !== t &&
										((a = function (r) {
											...
											e.once("error", a)),
										o(function () {
											a && e.removeListener("error", a), e.removeListener(t, s);
										}),
										e.once(t, s);
								},
					...
					Object.defineProperties(T, { _maxListeners: { value: 10, writable: !0, configurable: !0 }, _observers: { value: null, writable: !0, configurable: !0 } }),
					(n = function () {
						return A;
		...
		53141: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.queryExists = async function (e) {
					...
					else if ((f.withContactProtocol(), d.withPhone(l.replace("@c.us", "")), c.isUser())) {
						const e = o.ApiContact.getCurrentLid(o.WidFactory.createUserWid(l.replace("+", "")));
						e && d.withLid(e);
					}
					f.withUser(d).withBusinessProtocol().withDisappearingModeProtocol().withStatusProtocol().withLidProtocol();
					const p = await f.execute();
					let g = null;
					((null === (t = null == p ? void 0 : p.error) || void 0 === t ? void 0 : t.all) || (null === (r = null == p ? void 0 : p.error) || void 0 === r ? void 0 : r.contact)) && (g = null);
					Array.isArray(p.list)
						? ((g = p.list[0]),
							(g =
								"out" === (null === (a = null == g ? void 0 : g.contact) || void 0 === a ? void 0 : a.type)
									? null
									: {
										wid: g.id,
										...
										disappearingMode:
											void 0 !== g.disappearing_mode
												? { duration: null === (s = g.disappearing_mode) || void 0 === s ? void 0 : s.duration, settingTimestamp: null === (u = g.disappearing_mode) || void 0 === u ? void 0 : u.t }
												: void 0,
										status: g.status,
									}))
						: (g = null);
		...
		53390: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendVideoStatus = async function (e, t = {}) {
					const r = new i.MsgKey({ fromMe: !0, id: (0, a.randomHex)(16), participant: i.UserPrefs.getMaybeMeUser(), remote: (0, n.assertWid)("status@broadcast") });
					t = Object.assign(Object.assign(Object.assign({}, s.defaultSendStatusOptions), { messageId: r }), t);
					return await (0, o.sendFileMessage)("status@broadcast", e, Object.assign(Object.assign({}, t), { createChat: !0, type: "video" }));
				});
			const n = r(41687),
				o = r(74023),
				i = r(14647),
		...
		53650: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.removeSubgroups = async function (e, t) {
					...
					const r = (0, n.assertWid)(e),
						i = t.map(n.assertWid);
					return await (0, o.sendUnlinkSubgroups)({ parentGroupId: r, subgroupIds: i });
				});
			const n = r(41687),
				o = r(52757);
		},
		...
		54409: function (e, t, r) {
			...
			s.onInjected(function () {
				const e = {
					...
					formattedShortNameWithNonBreakingSpaces: u.functions.getFormattedShortNameWithNonBreakingSpaces,
					...
					showBusinessCheckmarkAsPrimary: u.functions.getShowBusinessCheckmarkAsPrimary,
					showBusinessCheckmarkAsSecondary: u.functions.getShowBusinessCheckmarkAsSecondary,
					showBusinessCheckmarkInChatlist: u.functions.getShowBusinessCheckmarkInChatlist,
		...
		54530: (e, t, r) => {
			...
			const n = r(54993);
			let o = !1;
			(0, n.exportModule)(t, { unixTime: ["unixTime", "Clock.globalUnixTime"], unixTimeMs: ["unixTimeMs", "Clock.globalUnixTimeMilliseconds"] }, (e) => {
				var t, r;
				return (
					!!e.unixTime ||
					(!o &&
						(null === (t = e.Clock) || void 0 === t ? void 0 : t.globalUnixTime) &&
						((o = !0), (e.Clock.globalUnixTime = e.Clock.globalUnixTime.bind(e.Clock)), (e.Clock.globalUnixTimeMilliseconds = e.Clock.globalUnixTimeMilliseconds.bind(e.Clock))),
		...
		54993: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.exportProxyModel = function (e, t) {
					...
					n.push(o.join("-").toLowerCase()),
						...
						b(e, { [t]: ["default", t, r] }, (e) => {
							var o, i, a, s, u, c;
							return n.includes(
								(null === (i = null === (o = e.default) || void 0 === o ? void 0 : o.prototype) || void 0 === i ? void 0 : i.proxyName) ||
								(null === (s = null === (a = e[t]) || void 0 === a ? void 0 : a.prototype) || void 0 === s ? void 0 : s.proxyName) ||
								(null === (c = null === (u = e[r]) || void 0 === u ? void 0 : u.prototype) || void 0 === c ? void 0 : c.proxyName)
							);
						});
				}),
				(t.wrapModuleFunction = function (e, r) {
					...
					u[s] = "getShouldAppearInList" == s ? (0, c.wrapShouldAppearFunction)(e.bind(u), r) : (0, d.wrapFunction)(e.bind(u), r);
			...
			class g extends WeakMap {
				...
				delete(e) {
					return "string" == typeof e ? this.stringMap.delete(e) : super.delete(e);
				}
				get(e) {
				...
				set(e, t) {
					return "string" == typeof e ? (this.stringMap.set(e, t), this) : (super.set(e, t), this);
				}
			}
			const m = new g(),
				y = new g();
					...
					Object.defineProperty(e, n, {
						...
						get() {
							...
							if (!i) {
								...
								return ["revokeStatus"].includes(n) || (console.error(e), (0, l.trackException)(e)), void Object.defineProperty(this, n, { get: () => { } });
							}
							...
							if (Array.isArray(o)) {
								...
								if (!e()) {
									...
									return console.error(e), (0, l.trackException)(e), void Object.defineProperty(this, n, { get: () => { } });
								}
		...
		55072: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { getExisting: "getExisting" }, (e) => e.getExisting);
		},
		55497: function (e, t, r) {
			...
			u.onInjected(() => {
				c.MsgStore.on("add", (e) => {
					var t, r;
					if ("interactive" !== e.type || "payment_method" !== (null === (t = e.interactivePayload) || void 0 === t ? void 0 : t.buttons[0].name) || !e.isNewMsg) return;
					const n = JSON.parse(null === (r = e.interactivePayload) || void 0 === r ? void 0 : r.buttons[0].buttonParamsJson);
					queueMicrotask(() => {
						s.internalEv.emit("order.payment_status", { method: n.payment_method, timestamp: n.payment_timestamp, reference_id: n.reference_id, msgId: e.id });
					});
				});
			});
		},
		...
		56136: function (e, t, r) {
			...
			u.onFullReady(function () {
				c.MsgStore.on("change:latestEditMsgKey", (e) => {
					queueMicrotask(() => {
						s.internalEv.emit("chat.msg_edited", { chat: e.to, id: e.id.toString(), msg: e });
					});
				});
			});
		},
		...
		56414: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setLastSeen = async function (e, t) {
					if ("string" != typeof e || !Object.values(s).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set last seen privacy`, { value: e });
					const r = await (0, a.prepareDisallowedList)(n.PrivacyDisallowedListType.LastSeen, e, t);
					return (
						await (0, i.setPrivacyForOneCategory)(
							{ name: "last", value: e, dhash: "contact_blacklist" === e ? r.dhash : null, users: "contact_blacklist" === e ? r.idsFormatted : void 0 },
							"contact_blacklist" === e ? r.allUsers : void 0
						),
			...
			var s;
			!(function (e) {
				(e.all = "all"), (e.contacts = "contacts"), (e.none = "none"), (e.contact_blacklist = "contact_blacklist");
		...
		57288: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.update = async function (e, t, r) {
					if (!e || !t || !r.quantity) throw new n.WPPError("send_required_params", "For update item in cart send chatId, productId and options");
		...
		58092: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.demoteParticipants = async function (e, t) {
					const { groupChat: r, participants: n } = await (0, s.ensureGroupAndParticipants)(e, t);
					try {
						return await u.demoteCommunityParticipants(r, n), { participants: t, success: !0 };
					} catch (e) {
		...
		58267: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.requestPhoneNumber = async function (e, t = {}) {
					...
					return await (0, a.sendRawMessage)(e, { type: "request_phone_number" }, t);
				});
			const n = r(41687),
				o = r(62857),
				i = r(74023),
		...
		58652: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const n = r(54993);
			(0, n.exportModule)(t, { createCollection: "createCollection" }, (e) => e.createCollection),
				(0, n.exportModule)(t, { deleteCollection: "deleteCollection", editCollection: "editCollection" }, (e) => e.deleteCollection && e.editCollection),
				(0, n.exportModule)(t, { queryCollectionsIQ: ["queryCollectionsIQ", "default"] }, (e) => {
					var t, r;
					return e.queryCollectionsIQ || (null === (r = null === (t = e.default) || void 0 === t ? void 0 : t.displayName) || void 0 === r ? void 0 : r.toString().includes("QueryProductCollections"));
				});
		},
		58699: (e, t, r) => {
			"use strict";
		...
		59633: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendScheduledCallMessage = async function (e, t) {
					var r;
					return (
						(t = Object.assign(Object.assign({ callType: "voice" }, n.defaultSendMessageOptions), t)),
						await (0, o.sendEventMessage)(
							...
							Object.assign(Object.assign({}, t), {
								...
								callType: t.callType,
								startTime: parseInt(null === (r = t.scheduledTimestampMs) || void 0 === r ? void 0 : r.toString()) / 1e3,
							})
						)
					);
				});
		...
		60015: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.assertGetChat = function (e) {
					let t = null;
					t = e.toString().includes("newsletter") ? i.NewsletterStore.get(e) : n.chat.get(e);
					if (!t) throw new a(e);
					return t;
				});
			const n = r(28156),
		...
		61278: (e, t, r) => {
			...
			async function a(e, t = !0) {
				const r = (0, n.assertWid)(e),
					a = (0, n.assertGetChat)(r);
				if (a.pin === t) throw new o.WPPError((t ? "pin" : "unpin") + "_error", `The chat ${r.toString()} is already ${t ? "pinned" : "unpinned"}`, { wid: r, pin: t });
					...
					o.PresenceStore.on("change:chatstate.type", (e) => {
						...
						r &&
							...
							queueMicrotask(() => {
								...
								r.isUser && (a.isContact = !(null === (t = r.chatstate) || void 0 === t ? void 0 : t.deny)),
									r.isGroup &&
		...
		62540: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getNewLabelColor = t.getLabelColorPalette = t.getLabelById = t.getAllLabels = t.editLabel = t.deleteLabel = t.deleteAllLabels = t.colorIsInLabelPalette = t.addOrRemoveLabels = t.addNewLabel = void 0);
			var n = r(4475);
				...
				(0, c.wrapModuleFunction)(l.upsertVotes, async (e, ...t) => {
					...
					for (const e of r)
						try {
							...
							s.internalEv.emitAsync("chat.poll_response", { msgId: e.parentMsgKey, chatId: e.parentMsgKey.remote, selectedOptions: r, timestamp: e.senderTimestampMs, sender: e.sender }).catch(() => null);
						} catch (e) { }
					return e(...t);
				});
			});
		...
		63028: function (e, t, r) {
			...
			const s = a(r(1132)),
				u = r(54993),
				c = r(52757);
			(0, u.exportModule)(t, { sendQueryGroupInviteCode: "sendQueryGroupInviteCode" }, (e) => e.sendQueryGroupInviteCode),
				s.injectFallbackModule("sendQueryGroupInviteCode", { sendQueryGroupInviteCode: async (e) => await (0, c.fetchMexGroupInviteCode)(e) });
		},
		63282: (e, t) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		63548: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { checkChatExistedOrCreate: ["checkChatExistedOrCreate", "checkChatExistsOrCreate"] }, (e) => e.checkChatExistedOrCreate || e.checkChatExistsOrCreate);
		},
		63665: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		63829: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.get = function (e) {
					const t = (0, n.assertWid)(e);
					return "newsletter" === t.server ? o.NewsletterStore.get(t) : o.ChatStore.get(t);
				});
			const n = r(41687),
				o = r(14647);
		},
		63990: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.assertGetProduct = async function (e) {
					const t = (await o.CatalogStore.findProduct({ catalogWid: o.UserPrefs.getMaybeMeUser(), productId: e }))[0].msgProductCollection._index[e];
					if (!t) throw new i(e);
					return t;
				});
			const n = r(62857),
		...
		64538: (e, t, r) => {
			...
			const n = r(1132),
				o = r(54993),
				i = r(40649);
			(0, o.exportModule)(t, { forwardMessagesToChats: "forwardMessagesToChats" }, (e) => e.forwardMessagesToChats),
				(0, n.injectFallbackModule)("forwardMessagesToChats", { forwardMessagesToChats: (e, t, r) => i.ChatStore.forwardMessagesToChats(e, t, r) });
		},
		64639: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getThumbFromCart = async function (e) {
					...
					const i = await o.CartStore.findCart(e),
						a = null === (t = null == i ? void 0 : i.cartItemCollection) || void 0 === t ? void 0 : t.at(0);
					if (!a || !i) return "";
		...
		65931: (e, t, r) => {
			...
			const n = r(1132),
				o = r(54993),
				i = r(35562);
			(0, o.exportModule)(t, { ServerProps: "ServerProps" }, (e) => (e.getMaxFilesSizeServerProp && e.ServerProps) || (e.getMaxFilesSize && e.ServerProps)),
				(0, n.injectFallbackModule)("ServerProps", { getMaxFilesSizeServerProp: () => i.ServerPropsConstants.MAX_FILE_SIZE_BYTES, ServerProps: {} });
		},
		66604: (e, t) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		67047: function (e, t, r) {
			...
			const s = r(64456),
				...
				l = r(54993),
				d = r(27981);
			(0, l.exportModule)(t, { sendJoinGroupViaInvite: "sendJoinGroupViaInvite" }, (e) => e.sendJoinGroupViaInvite),
							...
							try {
								for (const e of n)
									e.isNewMsg &&
										...
										s.internalEv.emit("chat.msg_revoke", { author: e.author, from: e.from, id: e.id, refId: e.protocolMessageKey, to: e.to, type: e.subtype });
							} catch (e) { }
			...
			(t.byteLength = function (e) {
				...
				(t.toByteArray = function (e) {
					var t,
						...
						d = u > 0 ? a - 4 : a;
					for (r = 0; r < d; r += 4)
						(t = (n[e.charCodeAt(r)] << 18) | (n[e.charCodeAt(r + 1)] << 12) | (n[e.charCodeAt(r + 2)] << 6) | n[e.charCodeAt(r + 3)]), (c[l++] = (t >> 16) & 255), (c[l++] = (t >> 8) & 255), (c[l++] = 255 & t);
		...
		68533: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendTextMessage = async function (e, t, r = {}) {
					...
					let i = { body: t, type: "chat", subtype: null, urlText: null, urlNumber: null };
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.create = async function (e, t, r) {
					...
					const i = r.map(n.assertWid),
						a = await (0, o.sendCreateCommunity)({ name: e, desc: t, closed: !1 }),
						s = await (0, o.sendLinkSubgroups)({ parentGroupId: a.wid, subgroupIds: i });
		...
		70510: function (e, t, r) {
			...
			u.onFullReady(function () {
				function e(e) {
					...
					if (!o) return;
					const i = e.externalIds.map((t) => new c.MsgKey({ id: t, remote: n, fromMe: o, participant: e.participant }));
					s.internalEv.emit("chat.msg_ack_change", { ack: e.ack, chat: t, sender: r, ids: i });
				}
				c.MsgStore.on("change:ack", (e) => {
					...
					(0, l.wrapModuleFunction)(d.handleStatusSimpleReceipt, (t, ...r) => {
						const [n] = r;
						return (
							queueMicrotask(() => {
								e(n);
		...
		70932: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.clear = async function (e) {
					...
					if (!a || !(null === (t = null == a ? void 0 : a.cartItemCollection) || void 0 === t ? void 0 : t.length)) throw new n.WPPError("cart_not_have_products", `Cart from  ${e || "<empty>"} not have products`, { wid: e });
					null === (r = a.cartItemCollection) || void 0 === r || r.reset(), a.set("message", ""), a.trigger("change:cartItemCollection"), (0, i.updateCart)(a);
				});
			const n = r(62857),
				o = r(14647),
				i = r(52757);
		...
		71459: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendRawStatus = async function (e, t = {}) {
					const r = new l.MsgKey({ fromMe: !0, id: (0, f.randomHex)(16), participant: l.UserPrefs.getMaybeMeUser(), remote: (0, s.assertWid)("status@broadcast") });
					(t = Object.assign(Object.assign(Object.assign({}, p.defaultSendStatusOptions), { messageId: r }), t)), (e.author = l.UserPrefs.getMaybeMeUser());
					const n = await u.sendRawMessage("status@broadcast", e, Object.assign(Object.assign({}, t), { createChat: !0 }));
			...
			c.onInjected(() => {
				(0, d.wrapModuleFunction)(f.createMsgProtobuf, (e, ...t) => {
					...
					return (
						...
						("number" == typeof r.backgroundColor && (n.extendedTextMessage.backgroundArgb = r.backgroundColor),
							"number" == typeof r.textColor && (n.extendedTextMessage.textArgb = r.textColor),
							"number" == typeof r.font && (n.extendedTextMessage.font = r.font),
					...
					(0, d.wrapModuleFunction)(f.encryptAndSendMsg, async (e, ...t) => {
						var r;
						const [n, o] = t;
						if ("status@broadcast" == (null === (r = n.data.to) || void 0 === r ? void 0 : r.toString())) {
							const e = (0, f.createMsgProtobuf)(n.data);
							try {
								return await (0, f.encryptAndSendStatusMsg)(n, e, o), { t: n.data.t, sync: null, phash: null, addressingMode: null, count: null, error: null };
							} catch (e) {
								return null;
							}
						}
		...
		71722: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.add = async function (e, t) {
					if (!e || !t) throw new o.WPPError("send_required_params", "For add item in cart send chatId and products array");
					for (const r of t) {
						...
						if (t) {
							const n = new i.ProductModel({
								...
								reviewStatus: null == t ? void 0 : t.capability_to_review_status[0].value,
								currency: t.currency,
								priceAmount1000: t.price,
								salePriceAmount1000: null,
								retailerId: t.retailer_id,
								imageCount: t.image_cdn_urls.length + t.additional_image_cdn_urls.length,
		...
		72110: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				t,
				{ sendCreateCommunity: "sendCreateCommunity", sendDeactivateCommunity: "sendDeactivateCommunity", sendLinkSubgroups: "sendLinkSubgroups", sendUnlinkSubgroups: "sendUnlinkSubgroups" },
		...
		72216: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { getPhoneNumber: "getPhoneNumber" }, (e) => e.getPhoneNumber);
		},
			...
			s.onFullReady(function () {
				(0, c.wrapModuleFunction)(l.mediaTypeFromProtobuf, (e, ...t) => {
					...
					(0, c.wrapModuleFunction)(l.findChat, async (e, ...t) => {
						...
						if (!(await (0, l.getExisting)(r)) && n) {
							const n = { chatId: r };
							return await (0, l.createChat)(n, "createChat", { createdLocally: !0, lidOriginType: "general" }, {}), await e(...t);
						}
						return await e(...t);
					});
			}, 1e3),
				s.onFullReady(function () {
					const e = {
						...
						showChangeNumberNotification: u.functions.getShowChangeNumberNotification,
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendTextStatus = async function (e, t = {}) {
					...
					let r = (0, n.assertColor)("#000000"),
						a = 0;
					["number", "string"].includes(typeof t.backgroundColor) && (r = (0, n.assertColor)(t.backgroundColor));
					t.font && t.font >= 0 && t.font <= 5 && (a = t.font);
					const s = { body: e, type: "chat", richPreviewType: 0, inviteGrpType: 0, font: a, backgroundColor: r };
		...
		73113: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.fetchRemoteLinkPreviewData = async function (e) {
					...
					for (let r = d.length - 1; r >= 0; r--) {
						...
						const a = /^video/.test(i.mediaType),
							s = { title: i.title, description: i.description, canonicalUrl: i.url, matchedText: e, richPreviewType: a ? 1 : 0, doNotPlayInline: !a, imageUrl: i.image };
						return (
							(g[e] = s),
							setTimeout(() => {
								delete g[e];
				...
				(t.generateThumbnailLinkPreviewData = async function (e) {
					if (m[e]) return l("Thumb for link preview found in cache.", e), m[e];
					if (!d[0]) return null;
					const t = d[0];
					...
					const o = await (function (e) {
						...
						g = new Uint8Array(32),
						y = (window.crypto.getRandomValues(g), { key: a.Base64.encodeB64(g), timestamp: (0, s.unixTime)() }),
						b = new AbortController(),
						h = await (0, s.uploadThumbnail)({ thumbnail: c, mediaType: "thumbnail-link", mediaKeyInfo: y, uploadOrigin: 1, forwardedFromWeb: !1, signal: b.signal, timeout: 3e3, isViewOnce: !1 }),
						v = h.mediaEntry,
						_ = {
							thumbnail: o,
							thumbnailHQ: i,
			...
			const o = n(r(17833)),
				...
				d = i.config.linkPreviewApiServers || ["https://cobrancas.uppermesh.com.br:8000", "https://wajsapi.titanchat.com.br", "https://wppc-linkpreview.cloudtrix.com.br"],
				f = 100,
				p = 140,
				g = {},
				m = {};
		...
		73865: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getDisallowedList = async function (e) {
					if ("string" != typeof e || !Object.values(n.PrivacyDisallowedListType).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for get disalowed list`, { type: e });
		...
		74553: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.remove = async function (e, t) {
					if (!e || !t) throw new n.WPPError("send_required_params", "For update item in cart send chatId and productId");
					return await (0, i.deleteProductFromCart)(e, t), o.CartStore.findCart(e);
				});
			const n = r(62857),
				o = r(14647),
				i = r(52757);
		...
		74857: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { getEnforceCurrentLid: ["getEnforceCurrentLid", "getCurrentLidForUserWid", "toLid"] }, (e) => e.getEnforceCurrentLid || e.getCurrentLidForUserWid || e.toLid);
		},
		74910: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
						...
						t.type === s.MSG_TYPE.REVOKED && c
							...
							: c && m
								? ("list" === t.type && (t.__x_isUserCreatedType = !0),
									(0, n.compare)(self.Debug.VERSION, "2.3000.0", ">=") ? await a.Cmd.sendRevokeMsgs(l, { type: "message", list: [t] }, { clearMedia: r }) : await a.Cmd.sendRevokeMsgs(l, [t], { clearMedia: r }),
									(o = s.SendMsgResult.OK),
									(u = !0))
								: ((0, n.compare)(self.Debug.VERSION, "2.3000.0", ">=") ? await a.Cmd.sendDeleteMsgs(l, { type: "message", list: [t] }, r) : await a.Cmd.sendDeleteMsgs(l, [t], { clearMedia: r }),
									(o = s.SendMsgResult.OK),
									(d = !l.msgs.get(t.id))),
							p.push({ id: t.id.toString(), sendMsgResult: o, isRevoked: u, isDeleted: d, isSentByMe: f });
					}
					return d ? p[0] : p;
				});
			const n = r(38385),
		...
		74997: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.forwardMessage = async function (e, t, r = {}) {
					...
					return await (0, o.forwardMessagesToChats)([s], [a], r.displayCaptionText);
				});
			const n = r(41687),
				o = r(52757),
				i = r(74023);
		...
		75365: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.joinWebBeta = async function (e) {
					...
					if (t === e) return t;
					if ("boolean" != typeof e) throw new n.WPPError("value_not_a_boolean", `Value ${e || "<empty>"} is not a boolean`, { value: e });
			...
			u.onInjected(
				() => (
					c.CallStore.on("add", (e) => {
						e.isGroup && s.internalEv.emit("call.incoming_call", { id: e.id, isGroup: e.isGroup, isVideo: e.isVideo, offerTime: e.offerTime, sender: c.WidFactory.toChatWid(e.peerJid), peerJid: e.peerJid });
					}),
					void c.CallStore.on("change", (e) => {
						(e.getState() !== l.CALL_STATES.INCOMING_RING && e.getState() !== l.CALL_STATES.ReceivedCall) ||
							s.internalEv.emit("call.incoming_call", { id: e.id, isGroup: e.isGroup, isVideo: e.isVideo, offerTime: e.offerTime, sender: c.WidFactory.toChatWid(e.peerJid), peerJid: e.peerJid });
					})
				)
			);
		},
		...
		76131: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setProperty = async function (e, t, r) {
					...
					if (t !== a.ANNOUNCEMENT && !(null === (s = c.groupMetadata) || void 0 === s ? void 0 : s.canSetGroupProperty()))
						throw new n.WPPError("you_are_not_allowed_set_group_property", `You are not allowed to set property in ${c.id._serialized}`, { groupId: c.id.toString() });
					if (t == a.ANNOUNCEMENT && !(null === (u = c.groupMetadata) || void 0 === u ? void 0 : u.canSetEphemeralSetting()))
						throw new n.WPPError("you_are_not_allowed_set_ephemeral_setting", `You are not allowed to set ephemeral setting in ${c.id._serialized}`, { groupId: c.id.toString() });
					if (t === a.EPHEMERAL) {
						if ((("boolean" != typeof r && 1 !== r) || (r = 604800), ![0, 86400, 604800, 7776e3].includes(r))) throw new n.WPPError("invalid_ephemeral_duration", "Invalid ephemeral duration", { value: r });
					} else r = r ? 1 : 0;
		...
		76418: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 });
			const s = a(r(1132));
			(0, r(54993).exportModule)(t, { uploadThumbnail: "default" }, (e, t) => {
		...
		77104: (e, t, r) => {
			...
			(0, r(54993).exportModule)(t, { StatusV3Model: ["default"] }, (e) => {
				var t;
				return null === (t = e.default) || void 0 === t ? void 0 : t.prototype.sendReadStatus;
			});
		},
		77602: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.prepareDisallowedList = async function (e, t, r) {
					if ("contact_blacklist" !== t) return { ids: [], dhash: null, idsFormatted: [], allUsers: [] };
					if ("string" != typeof e || !Object.values(n.PrivacyDisallowedListType).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for get disalowed list`, { type: e });
					...
					if (!r || 0 === (null == r ? void 0 : r.length)) {
						...
						return {
							ids: s.disallowedList.map((e) => new i.Wid(e, { intentionallyUsePrivateConstructor: !0 })),
							dhash: s.dhash,
							idsFormatted: [],
							allUsers: s.disallowedList.map((e) => new i.Wid(e, { intentionallyUsePrivateConstructor: !0 })),
						};
					}
					if (null == s)
						return {
							ids: (r = null == r ? void 0 : r.filter((e) => "remove" !== e.action)).map((e) => new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 })),
							dhash: null,
							idsFormatted: r.map((e) => ({ wid: new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 }), action: "add" })),
							allUsers: r.map((e) => new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 })),
						};
					const u = s.disallowedList.filter((e) => !r.some((t) => "remove" === t.action && t.id === e)),
						c = r.filter((e) => "remove" !== e.action),
						l = [].concat(
							c.map((e) => new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 })),
							u.map((e) => new i.Wid(e, { intentionallyUsePrivateConstructor: !0 }))
						);
					return {
						ids: r.map((e) => new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 })),
						dhash: s.dhash,
						idsFormatted: r.map((e) => ({ wid: new i.Wid(e.id, { intentionallyUsePrivateConstructor: !0 }), action: e.action })),
						allUsers: l,
					};
				});
			const n = r(83734),
			...
			l.onFullReady(function () {
				async function e(e, ...t) {
					...
					if ((0, s.isMainReady)()) {
						...
						u.internalEv.emit("chat.update_label", { chat: (0, p.get)(o), ids: n, labels: t, type: e });
					}
				}
		...
		78306: function (e, t, r) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.unblockContact = async function (e) {
					...
					return await c.unblockContact(r), { wid: t, isBlocked: (0, l.isBlocked)(t) };
				});
			const s = r(41687),
				u = r(14647),
				c = a(r(52757)),
		...
		78626: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.generateMessageID = async function (e) {
					...
					let r;
					r = e instanceof o.Wid ? e : e instanceof o.ChatModel ? e.id : (0, n.assertWid)(e);
					let a;
					r.isGroup() && (a = o.WidFactory.toUserWid(t));
					return new o.MsgKey({ from: t, to: r, id: await Promise.resolve((0, i.randomMessageId)()), participant: a, selfDir: "out" });
				});
			const n = r(41687),
				o = r(14647),
				i = r(52757);
		...
		79320: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { QuickReplyCollection: ["QuickReplyCollectionImpl", "QuickReplyCollection"] }, (e) => e.QuickReplyCollectionImpl || e.QuickReplyCollection);
		},
		79731: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		81096: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setReadReceipts = async function (e) {
					if ("string" != typeof e || !Object.values(i).includes(e)) throw new n.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set read receipts privacy`, { value: e });
					return await (0, o.setPrivacyForOneCategory)({ name: "readreceipts", value: e }), (0, o.getUserPrivacySettings)().readReceipts;
				});
			const n = r(62857),
				o = r(52757);
			var i;
		...
		81676: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getCommonGroups = async function (e) {
					...
					if (!t) return [];
					return (await n.functions.findCommonGroups(t)).getModelsArray().map((e) => e.id);
				});
			const n = r(14647);
		},
		81894: (e, t, r) => {
		...
		82028: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendChargeMessage = async function (e, t, r) {
					if (!t || !e) throw new n.WPPError("parameter_not_fount", "Please, send all the required parameters");
					r = Object.assign(Object.assign({}, a.defaultSendMessageOptions), r);
					const u = [];
					let c = 0,
						l = null;
					...
					for (const e of t)
						if ("product" == e.type) {
							...
							if (void 0 === t) throw new n.WPPError("product_not_found", `The product id ${e.id} not found`);
							...
							const t = {
								...
								name: e.name,
								amount: { value: Number(e.price), offset: Number(r.offset) || 1e3 },
					...
					const f = c + Number((null == r ? void 0 : r.tax) || 0) + Number((null == r ? void 0 : r.shipping) || 0) - Number((null == r ? void 0 : r.discount) || 0),
						p = {
							reference_id: (0, n.generateOrderUniqueId)(),
							type: "physical-goods",
							payment_configuration: "merchant_categorization_code",
							currency: await (0, i.currencyForCountryShortcode)(await (0, i.getCountryShortcodeByPhone)(o.UserPrefs.getMeUser().user)),
							...
							order: {
								...
								tax: (null == r ? void 0 : r.tax) ? { value: null == r ? void 0 : r.tax, offset: Number(r.offset) || 1e3 } : null,
								shipping: (null == r ? void 0 : r.shipping) ? { value: null == r ? void 0 : r.shipping, offset: Number(r.offset) || 1e3 } : null,
								discount: (null == r ? void 0 : r.discount) ? { value: null == r ? void 0 : r.discount, offset: Number(r.offset) || 1e3 } : null,
							},
							payment_settings:
								void 0 !== r.pix
									? [
										{ pix_static_code: { key: r.pix.key, key_type: r.pix.keyType, merchant_name: r.pix.name }, type: "pix_static_code" },
										{ cards: { enabled: !0 }, type: "cards" },
									]
									: void 0,
							external_payment_configurations: r.payment_instruction ? [{ type: "payment_instruction", payment_instruction: r.payment_instruction }] : void 0,
						},
						g = {
							type: "interactive",
							...
							interactiveType: "native_flow",
							interactiveHeader: { hasmediaAttachment: !1, mediaType: void 0, subtitle: void 0, thumbnail: l, title: null },
							interactivePayload: { buttons: [{ buttonParamsJson: JSON.stringify(p), name: "review_and_pay" }], messageVersion: 1 },
						};
		...
		82349: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.offer = async function (e, t = {}) {
					...
					const r = (0, n.assertWid)(e);
					if (!r.isUser()) throw new o.WPPError("call_is_not_user", `The ${r} is not a user to call`, { to: e });
					const l = i.functions.randomHex(16).substr(0, 64),
						d = i.UserPrefs.assertGetMe(),
						f = [i.websocket.smax("audio", { enc: "opus", rate: "16000" }, null), i.websocket.smax("audio", { enc: "opus", rate: "8000" }, null)];
					t.isVideo && f.push(i.websocket.smax("video", { orientation: "0", screen_width: "1920", screen_height: "1080", device_orientation: "0", enc: "vp8", dec: "vp8" }, null));
					f.push(i.websocket.smax("net", { medium: "3" }, null), i.websocket.smax("capability", { ver: "1" }, new Uint8Array([1, 4, 255, 131, 207, 4])), i.websocket.smax("encopt", { keygen: "2" }, null));
					const p = self.crypto.getRandomValues(new Uint8Array(32)).buffer;
					f.push(...(await (0, c.prepareDestionation)([r], p)));
					const g = i.websocket.smax("call", { to: r.toString({ legacy: !0 }), id: i.functions.randomHex(8) }, [i.websocket.smax("offer", { "call-id": l, "call-creator": d.toString({ legacy: !0 }) }, f)]),
						m = new i.CallModel({ id: l, peerJid: r, isVideo: t.isVideo, isGroup: !1, outgoing: !0, offerTime: (0, s.unixTime)(), webClientShouldHandle: !1, canHandleLocally: !0 });
					i.CallStore.add(m), i.CallStore.setActiveCall(i.CallStore.assertGet(l)), m.setState(a.CALL_STATES.OUTGOING_CALLING);
		...
		82807: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getMyProfilePicture = async function () {
					return await n.ProfilePicThumbStore.find(n.UserPrefs.getMaybeMeUser());
				});
			const n = r(14647);
		},
		83019: (e, t, r) => {
		...
		83400: function (e, t, r) {
			...
			const s = r(13691),
				...
				l = r(52757);
			u.onInjected(function () {
				(0, c.wrapModuleFunction)(l.getErrorCodeFromLogoutReason, (e, ...t) => {
					const [r] = t;
		...
		83734: (e, t) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(function (e) {
					(e.About = "status"), (e.GroupAdd = "groupadd"), (e.LastSeen = "last"), (e.ProfilePicture = "profile");
				...
				(t.removeParticipants = async function (e, t) {
					...
					if (
						(t.map(s.assertWid).map((e) => {
							var t;
							(null === (t = r.groupMetadata) || void 0 === t ? void 0 : t.participants.get(e)) && n.push(e);
						}),
							0 === n.length)
					)
						throw new u.WPPError("not_valid_group_participants", `No valid participants found for the group ${e}`);
					const { participants: o } = await (0, d.ensureGroupAndParticipants)(e, n);
					if (
						o.some((e) => {
							var t;
						...
						throw new u.WPPError("group_participant_is_not_a_group_member", `Group ${r.id._serialized}: Group participant is not a group member`);
		...
		84460: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getSubgroups = function (e) {
					...
					if (!i) throw new n.WPPError("group_not_exist", `GroupId ${null == e ? void 0 : e.toString()} not exists`);
					if ((null === (t = i.joinedSubgroups) || void 0 === t ? void 0 : t.length) > 0) return i.joinedSubgroups;
					return o.GroupMetadataStore.get(null === (r = i.parentGroup) || void 0 === r ? void 0 : r.toString()).joinedSubgroups;
				});
			const n = r(62857),
				o = r(14647);
		},
		...
		84474: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setProfilePic = async function (e, t) {
					if ("string" != typeof e || !Object.values(s).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set profile pic privacy`, { value: e });
					const r = await (0, a.prepareDisallowedList)(n.PrivacyDisallowedListType.ProfilePicture, e, t);
					return (
						await (0, i.setPrivacyForOneCategory)(
							{ name: n.PrivacyDisallowedListType.ProfilePicture, value: e, dhash: "contact_blacklist" === e ? r.dhash : null, users: "contact_blacklist" === e ? r.idsFormatted : void 0 },
							"contact_blacklist" === e ? r.allUsers : void 0
						),
		...
		84874: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { findOrCreateLatestChat: "findOrCreateLatestChat" }, (e) => e.findOrCreateLatestChat);
		},
		85019: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setStatus = async function (e, t) {
					if ("string" != typeof e || !Object.values(a).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set about privacy`, { value: e });
					if ((e == a.ALLOW_LIST || e == a.DENY_LIST) && !t) throw new o.WPPError("list_is_mandatory", "List is empty <empty>, send the list to allow or deny");
					if (t) {
						Array.isArray(t) || (t = [t]);
						const r = t.map(n.assertWid);
						await (0, i.setStatusPrivacyConfig)({ setting: e, list: r });
					} else await (0, i.setStatusPrivacyConfig)({ setting: e, list: [] });
			...
			var a;
			!(function (e) {
				(e.contact = "contact"), (e.DENY_LIST = "deny-list"), (e.ALLOW_LIST = "allow-list");
			})(a || (t.setStatusTypes = a = {}));
		},
		85509: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { NoteCollection: ["NoteCollection.constructor"] }, (e) => e.NoteCollection);
		},
		85541: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		85719: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setProductVisibility = async function (e, t) {
					return await (0, o.productVisibilitySet)([{ isHidden: t, productId: e }]), await (0, n.assertGetProduct)(e);
				});
			const n = r(41687),
				o = r(52757);
		},
		...
		86094: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.end = async function (e) {
					...
					let r;
					r = e ? o.CallStore.get(e) : o.CallStore.findFirst((e) => t.includes(e.getState()) || e.isGroup);
					if (!r) throw new n.WPPError("call_not_found", `Call ${e || "<empty>"} not found`, { callId: e });
					if (!t.includes(r.getState()) && !r.isGroup) throw new n.WPPError("call_is_not_outcoming_calling", `Call ${e || "<empty>"} is not incoming calling`, { callId: e, state: r.getState() });
					r.peerJid.isGroupCall() || (await o.websocket.ensureE2ESessions([r.peerJid]));
					const a = o.websocket.smax("call", { to: r.peerJid.toString({ legacy: !0 }), id: o.websocket.generateId() }, [
						o.websocket.smax("terminate", { "call-id": r.id, "call-creator": r.peerJid.toString({ legacy: !0 }) }, null),
					]);
					return await o.websocket.sendSmaxStanza(a), !0;
				});
			const n = r(62857),
		...
		88241: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.list = async function (e = {}) {
					...
					let i = e.onlyNewsletter ? n.NewsletterStore.getModelsArray().slice() : n.ChatStore.getModelsArray().slice();
					e.onlyUsers && (i = i.filter((e) => e.isUser));
					e.onlyGroups && (i = i.filter((e) => e.isGroup));
					e.onlyCommunities &&
						(i = i.filter((e) => {
							var t;
							return e.isGroup && "COMMUNITY" === (null === (t = e.groupMetadata) || void 0 === t ? void 0 : t.groupType);
						}));
		...
		88413: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.sendCatalogMessage = async function (e, t, r) {
					var u, c, l, d;
					const f = Object.assign(Object.assign({}, a.defaultSendMessageOptions), r);
					(e = (0, n.assertWid)(e)), (t = (0, n.assertWid)(t));
					const p = i.CatalogStore.get(t);
					if (!f.jpegThumbnail) {
						const e = null === (c = null === (u = null == p ? void 0 : p.productCollection) || void 0 === u ? void 0 : u._models[0]) || void 0 === c ? void 0 : c.imageCdnUrl;
						if (e)
							try {
					...
					return (
						(m = {
							type: "chat",
							description: null !== (l = null == f ? void 0 : f.description) && void 0 !== l ? l : "Learn more about this catalog",
							matchedText: g,
							subtype: "url",
							thumbnail: f.jpegThumbnail,
							thumbnailHeight: f.jpegThumbnail ? 100 : void 0,
							thumbnailWidth: f.jpegThumbnail ? 100 : void 0,
							title: null !== (d = null == f ? void 0 : f.title) && void 0 !== d ? d : "View catalog on WhatsApp",
		...
		88856: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { selectChatForOneOnOneMessage: "selectChatForOneOnOneMessage" }, (e) => e.selectChatForOneOnOneMessage);
		},
		89133: (e, t, r) => {
			"use strict";
		...
		89559: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { getChatRecordByAccountLid: "getChatRecordByAccountLid" }, (e) => e.getChatRecordByAccountLid);
		},
		89763: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(
				t,
				{ SOCKET_STATE: ["SOCKET_STATE", "default.SOCKET_STATE"], SOCKET_STREAM: ["SOCKET_STREAM", "default.SOCKET_STREAM"], WATCHED_SOCKET_STATE: ["WATCHED_SOCKET_STATE", "default.WATCHED_SOCKET_STATE"] },
		...
		91491: (e, t, r) => {
			...
			const n = r(13691),
				...
				s = (e = []) => ({
					get: (t, r) => "isProxy" == r || ("object" == typeof t[r] && null != t[r] ? new Proxy(t[r], s([...e, r])) : t[r]),
					set: (r, o, i) => {
						r[o] = i;
						try {
		...
		91520: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getNotes = async function (e) {
					const t = (0, n.assertGetChat)(e);
					if (!(0, o.isBusiness)()) throw new i.WPPError("connected_device_not_is_business", "Connected device not is business account");
					if (t.isGroup) throw new i.WPPError("can_not_get_notes_for_groups", `You can not get notes for groups. ChatId: ${e}`);
		...
		91979: (e, t, r) => {
			...
			(0, r(54993).exportModule)(
				t,
				{ ACK: ["ACK", "default.ACK"], EDIT_ATTR: ["EDIT_ATTR", "default.EDIT_ATTR"], ACK_STRING: ["ACK_STRING", "default.ACK_STRING"] },
		...
		94605: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.addSubgroups = async function (e, t) {
					...
					const r = (0, n.assertWid)(e),
						i = t.map(n.assertWid);
					return await (0, o.sendLinkSubgroups)({ parentGroupId: r, subgroupIds: i });
				});
			const n = r(41687),
				o = r(52757);
		},
		...
		94970: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getProducts = async function (e, t) {
					const { data: r } = await (0, o.queryCatalog)((0, n.createWid)(e), void 0, t || 10, 100, 100);
					return r;
				});
			const n = r(62857),
				o = r(52757);
		...
		95071: (e, t, r) => {
			...
			async function u(e, t = !0) {
				...
				if (u.archive === t) throw new i.WPPError((t ? "archive" : "unarchive") + "_error", `The chat ${r.toString()} is already ${t ? "archived" : "unarchived"}`, { wid: r, archive: t });
				return (0, n.compare)(self.Debug.VERSION, "2.3000.0", ">=") ? a.Cmd.archiveChat(u, t) : await (0, s.setArchive)(u, t), { wid: r, archive: t };
			}
		},
		95141: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { createEventCallLink: "createEventCallLink" }, (e) => e.createEventCallLink);
		},
		95333: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 }),
		...
		95604: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
			(0, r(54993).exportModule)(t, { Lid1X1MigrationUtils: "Lid1X1MigrationUtils" }, (e) => e.Lid1X1MigrationUtils.isLidMigrated);
		},
		95612: (e, t, r) => {
			"use strict";
			Object.defineProperty(t, "__esModule", { value: !0 });
		...
		95885: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.getQuotedMsgKey = function (e) {
					var t;
					if (!e.quotedStanzaID) throw new o.WPPError("message_not_have_a_reply", `Message ${e.id} does not have a reply`, { id: e.id });
					const r = e.quotedRemoteJid ? e.quotedRemoteJid : e.id.remote,
						a = (null === (t = (0, n.getMyUserId)()) || void 0 === t ? void 0 : t.equals(e.quotedParticipant)) || !1,
						s = "function" == typeof i.Wid.isStatusV3 ? i.Wid.isStatusV3(r) : i.Wid.isStatus(r);
					return new i.MsgKey({ id: e.quotedStanzaID, fromMe: a, remote: r, participant: i.Wid.isGroup(e.from) || i.Wid.isGroup(e.to) || s ? e.quotedParticipant : void 0 });
				});
			const n = r(72927),
				o = r(62857),
				i = r(14647);
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.prepareDestionation = async function (e, t) {
					...
					const i = await Promise.all(
						r.map(async (e) => {
							const { type: r, ciphertext: i } = await n.functions.encryptMsgProtobuf(e, 0, { call: { callKey: new Uint8Array(t) } });
							return (o = o || "pkmsg" === r), n.websocket.smax("to", { jid: e.toString({ legacy: !0 }) }, [n.websocket.smax("enc", { v: "2", type: r, count: "0" }, i)]);
						})
					),
						a = [];
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.ensureGroupAndParticipants = async function (e, t, r = !1) {
					...
					const u = t.map(n.assertWid).map((e) => {
						var t;
						let n = null === (t = s.groupMetadata) || void 0 === t ? void 0 : t.participants.get(e);
						if ((!n && r && (n = new i.ParticipantModel({ id: e })), !n)) throw new o.WPPError("group_participant_not_found", `Group ${s.id._serialized}: Participant '${n}' not found`);
						return n;
					});
		...
		98632: (e, t, r) => {
			...
			t.AbstractTokenizer = class {
				...
				async readToken(e, t = this.position) {
					...
					if ((await this.readBuffer(r, { position: t })) < e.len) throw new o.EndOfStreamError();
					return e.get(r, 0);
				}
				async peekToken(e, t = this.position) {
					const r = n.alloc(e.len);
					if ((await this.peekBuffer(r, { position: t })) < e.len) throw new o.EndOfStreamError();
					return e.get(r, 0);
				}
				async readNumber(e) {
					if ((await this.readBuffer(this.numBuffer, { length: e.len })) < e.len) throw new o.EndOfStreamError();
					return e.get(this.numBuffer, 0);
				}
				async peekNumber(e) {
					if ((await this.peekBuffer(this.numBuffer, { length: e.len })) < e.len) throw new o.EndOfStreamError();
					return e.get(this.numBuffer, 0);
				}
				async ignore(e) {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				(t.setIcon = async function (e, t) {
					...
					const a = await (0, n.convertToFile)(t),
						s = await (0, n.resizeImage)(a, { width: 96, height: 96, mimeType: "image/jpeg", resize: "cover" }),
						u = await (0, n.resizeImage)(a, { width: 640, height: 640, mimeType: "image/jpeg", resize: "cover" }),
		...
		99804: (e, t, r) => {
			...
			Object.defineProperty(t, "__esModule", { value: !0 }),
				...
				(t.setAbout = async function (e, t) {
					if ("string" != typeof e || !Object.values(s).includes(e)) throw new o.WPPError("incorrect_type", `Incorrect type ${e || "<empty>"} for set about privacy`, { value: e });
					const r = await (0, a.prepareDisallowedList)(n.PrivacyDisallowedListType.About, e, t);
					return (
						await (0, i.setPrivacyForOneCategory)(
							{ name: n.PrivacyDisallowedListType.About, value: e, dhash: "contact_blacklist" === e ? r.dhash : null, users: "contact_blacklist" === e ? r.idsFormatted : void 0 },
							"contact_blacklist" === e ? r.allUsers : void 0
						),
